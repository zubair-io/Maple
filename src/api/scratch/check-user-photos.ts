...
async function run() {
  ...
  const filenames = ['_84A2380.CR2', '_84A2381.CR2'];
  const docs = await assets.find({ 'fileinfo.filename': { $in: filenames } }).toArray();

  console.log('--- DB State for Zoo Photos ---');
  for (const doc of docs) {
    const primary = assetActiveFileInfo(doc as any);
    const target = geoDir(doc);
    const wouldMove = wouldRelocate(doc);
    console.log(`File: ${doc.fileinfo?.[0]?.filename}`);
    console.log(`  fileinfo:`, JSON.stringify(doc.fileinfo));
    console.log(`  metadata_override:`, JSON.stringify(doc.metadata_override));
    console.log(`  place:`, JSON.stringify(doc.place));
    console.log(`  exif:`, JSON.stringify(doc.exif));
    console.log(`  is_screenshot:`, doc.is_screenshot);
    console.log(`  Would Relocate: ${wouldMove}`);
