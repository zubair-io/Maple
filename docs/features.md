...
`whiteBalancePreset` is a sidecar marker outside the numerical adjustment schema. Apple and Web offer its full vocabulary: As Shot, Auto, Daylight, Cloudy, Shade, Tungsten, Fluorescent, Flash and Custom. Named illuminants apply the shared core’s temperature/tint pair and record Preset provenance as one undoable edit. The Auto choice applies only the corrected white-balance estimate, preserving tone and AE. Web also offers a two-dimensional WB pad; both platforms provide temperature/tint sliders and an As Shot restore. Preset definitions are generated from the same resolver that reads name-only XMP sidecars; see [the WB contract](xmp-canonical-format.md) for calibration and reference coverage.
