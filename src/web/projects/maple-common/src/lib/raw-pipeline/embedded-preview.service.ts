...
export class EmbeddedPreviewService implements OnDestroy {
  ...
  private ensureWorker(): Worker {
    ...
    // Recover from a worker-level failure (script load failure, uncaught
    // exception): reject every in-flight extraction, terminate the broken
    // instance, and drop our reference so the next `ensureWorker()` spins up
    // a fresh one. Same inline shape as
    // `MapleIdFallbackHasherService.ensureWorker()` (the identical
    // dedicated-single-purpose-worker pattern) — deliberately NOT factored
    // into a shared helper for a single second caller, per this repo's YAGNI
    // convention.
    worker.addEventListener('error', (event) => {
      const err = new Error(`embedded-preview worker error: ${event.message || 'unknown error'}`);
