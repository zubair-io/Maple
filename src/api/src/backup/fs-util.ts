  ...
  try {
    ...
    try {
      ...
      while (pos < sa.size) {
        ...
        if (
          !bufA.subarray(0, ra.bytesRead).equals(bufB.subarray(0, rb.bytesRead))
        ) {
          return false;
        }
        pos += ra.bytesRead;
...
export async function firstFreeSiblingPath(
  ...
  for (let i = 1; i <= 9999; i++) {
    ...
    try {
      ...
      if (e?.code === "ENOENT")
        return { relPath: candidateRel, absPath: candidateAbs };
      throw e;
    }
  }
  throw new Error(
    `could not find a free destination for ${relPath} after 9999 attempts`,
  );
}
