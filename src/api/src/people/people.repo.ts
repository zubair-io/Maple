...
/** Hex ids for the display names in search's `people` filter (#2864).
 ...
 * of the facets `people` bucket (#2864): the faces aggregation groups by
 * `person_id` and this maps the surviving ids to names. Hidden and
 * merged-away persons are omitted so they drop out of the picker. */
export async function namesForPersonIds(hexIds: string[]): Promise<Map<string, string>> {
  if (hexIds.length === 0) return new Map();
