...
**The third container, per host (#3271, #3282, #3300).** `crs:MaskGroupBasedCorrections` is modeled wherever a host carries the `bitmap` / `everywhere` mask variants on its model, and only a host with a raster _source_ can actually render the masked pixels:

| Host    | Model (`bitmap` + `everywhere`)         | Wire (reads + writes the container)            | Raster registry                                                                   | Raster source                                                       |
| ------- | --------------------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Rust    | `Mask::Bitmap` / `Mask::Everywhere`     | `raw_core::xmp::local_adjustments` (reference) | `raw-ffi` `maple_mask_raster_register/release`                                    | none (a host supplies the raster)                                   |
| Apple   | `LocalMask.bitmap` / `.everywhere`      | `XMPSerialization+LocalAdjustments.swift`      | via raw-ffi                                                                       | Vision person/skin segmentation (#3284)                             |
| Web     | `LocalMask` `'bitmap'` / `'everywhere'` | `xmp-local-adjustments.ts`                     | `raw-wasm` `mask_raster_register/release` (`register-mask-raster` worker message) | **none yet** — the segmentation source is #3300's remaining slice 3 |
| Windows | —                                       | passthrough (§ "Passthrough", bucket 2)        | —                                                                                 | —                                                                   |

The three writers that model it pin one shared bitmap + everywhere literal byte-for-byte (`tests_local_adjustments_bitmap.rs`, `XMPLocalAdjustmentsTests.swift`, `local-adjustments-bitmap.spec.ts`), the same contract as the two-layer geometric fixture above. On Windows the container still rides the generic unknown-child-element passthrough byte-for-byte, whatever it contains. Rust and Apple regenerate the container from their model, so an unrecognized correction does not survive their parse-then-serialize path. Web instead preserves opaque group corrections alongside modeled Maple bitmap/everywhere layers (#3423): foreign AI recipes, composite masks, inactive pins and incomplete recipes are retained, including their container metadata, namespace bindings, comments and relative positions. A group template stores opaque XML and replacement slots for owned corrections; changing or deleting Maple layers cannot erase a foreign correction, and newly added layers append within the existing group. Groups in secondary RDF descriptions with the same subject and XML context follow the same path; other subjects, contexts and nested RDF trees remain opaque in their original wrappers. Owned entries carry a host-only slot identifier so deleting an earlier layer cannot move a surviving one across a foreign correction; it is never written to XMP. As with existing Web passthrough, import uses DOM serialization (quote/entity spellings may normalize), then preserved subtrees and subsequent whole-document saves are fixed points. The canonical fixture for a purely modeled group remains unchanged. A web sidecar carrying a `PersonSkin` mask renders that layer at weight 0 (its other edits intact) until a raster is registered for its digest — see `docs/web.md` § "The render worker".
