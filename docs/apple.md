...
Everything Apple lives under `src/apple/`: one Xcode project (`Maple.xcodeproj`) that ships a single universal app — product name **Maple**, bundle id `app.justmaple.aperture`, running on macOS, iPhone and iPad from one target — plus a separate tvOS app, four extensions, a macOS LaunchAgent, and two test bundles. Almost none of the interesting code is in the Xcode targets: the domain lives in three local Swift packages under `src/apple/Packages/` (`MapleCore`, `MapleUI`, `MapleBackup`), and all the image math lives in Rust, linked as a static library through `Frameworks/RawPipeline.xcframework`. Those `.a` slices and the generated `RawPipeline.h` headers are gitignored (the static libs are 200–500 MB each), so a fresh clone must build them once with `scripts/build-xcframework.sh` before Xcode can link or SwiftPM can compile. The editor renders through the Rust FFI — either the wgpu GPU path presenting straight into a `CAMetalLayer`, or a CoreImage CPU fallback — never through hand-written Apple render kernels, which were all deleted once the Rust chain subsumed them.
...
Every committed editor action is one `EditTransaction` (`Editor/EditTransaction.swift`): a per-session id, the action class (`adjustment`, `auto`, `crop`, `paste`, `preset`, `reset`, plus `mask` / `repair` / `variant` reserved for the surfaces that ship them), a user-visible description, the semantic before/after model, a deterministic sidecar diff (canonical XMP attribute keys and values, omit-on-default on both platforms), and the render-invalidation scope that diff implies (`crop` / `develop` / `decode`). `EditSession.beginEdit(kind:description:)` opens one, the gesture's per-tick model writes are previews (render + coalesced sidecar write, no history), and `endEdit()` — reached from `EditorState.endGesture()`, from every discrete edit, or lazily at the next boundary (`beginEdit`, `undo`, `redo`, `flushPendingSidecarWrite`) — closes it: a no-op is dropped, anything else becomes exactly one entry on the 32-deep undo ring (`EditSession+UndoRedo.swift`), is handed to the sidecar store as `after`, and is spoken through `EditAnnouncer` (an accessibility announcement by default). Undo and redo swap `before`/`after` and announce "Undo …" / "Redo …". The web `EditorStateService` mirrors the same object by hand; `EditTransactionTests` and `EditSessionTransactionTests` pin the diff format, the one-entry-per-action rule per action class, the real-`.xmp` reload round-trip, and the stale-render guard.
...
Editor histograms reuse the **presented preview** (#3360), rather than independently decoding/developing the RAW. The Apple GPU present encoder bins a bounded grid (at most 256×256 samples) using the same display quantizer; its buffers and bind groups are created once per session, and only 768 `u32` counts (3 KB) are read back after the 350 ms UI debounce. Preview-cache/export readbacks cannot overwrite those statistics. The CPU canvas bins a small raster of its already-cropped `renderedPreview` in the selected canvas color space. `LiveHistogramState` coalesces the pill and inspector onto one request/result per presented-frame revision; new frames cancel obsolete work and invalidate that result. `DisplayedHistogramTests`, `LiveHistogramStateTests`, and the Rust display-histogram tests cover source-read avoidance, crop/color/exposure, shared requests, stale-result rejection, quantization parity, and bounded samples/readback timing.

The editor's scopes beyond the histogram — luma waveform, RGB parade, vectorscope (#3251) — ride on the vectorscope scope pass (#3272). While either scope surface is showing, the GPU-live present encodes, beside the 128×128 Cb/Cr histogram, a box-mean RGB8 snapshot of the display-encoded frame (long edge clamped to 512 px — `raw-gpu/src/scope_snapshot.wgsl`, twin of `raw_core::scope::snapshot`) into the same double-buffered staging slot, and `take_scope_stats` reports both one tick late without ever blocking a present; the CPU fallback (`EditSession+ScopeCpu.swift` → `maple_apply_chain_and_encode_display_scoped_f32`) produces the same pair synchronously after its 350 ms debounce. `MapleScopeStats` carries the snapshot through a host-owned `snapshot_ptr` buffer (`GpuLiveSession+Scope.swift`, `PipelineRenderer+Scoped.swift`) into `ScopeSample.snapshot`; `ScopePanelSample.reduce` turns it into the 64-column luma waveform, RGB parade and 64-bin histograms — the same reduction as the web's `scope-sample.ts` — in a detached task keyed on the sample's frame, so the reduction never runs on the present path. `EditorScopesPanel` mounts `MuiScopesPanel` over it (top-leading beside the filmstrip rail on regular width, pill "Scopes" toggle, `editor-scopes-panel`), drawing the vectorscope from the density bins rather than a dot sample; the skin-tone HUD (`VectorscopeHud`, pill "Scope" toggle) stays the compact-width and mask-workflow surface. `EditorView` arms `session.scopeEnabled` while either is showing. `ScopePanelSampleTests` pin the reduction and its 50 ms ceiling on a 512×512 snapshot; `ScopeCpuProducerTests` push real pixels through the CPU producer's snapshot; raw-gpu's `scope/tests.rs` gates the WGSL downsample against raw-core within 1 LSB.

Standalone `LocalHistogram.compute` and `computeNonRaw` still share one process-wide slot, acquired before source reads and held until synchronous native processing returns. Their cancellation regressions remain in `LocalHistogramCancellationTests`; the editor no longer calls those full-develop paths.
...
- The legacy tile compositor is **off by default** (`EditSession.deepZoomEnabled` is `false`). Local, uncropped RAWs at native zoom first attempt the bounded native-detail path; otherwise refine uses the whole-image sized FFI. The flag gates the legacy tile fallback, not native-detail rendering, and the tile fallback additionally stays off while a mask stack exists (its tiles carry no per-tick chain). See [zoom](zoom.md).
- Masks are evaluated by the Rust core in full-frame normalized coordinates, and the CPU refine and export feed it the whole frame with the crop applied afterwards. The two paths that hand the chain a partial frame — the GPU-live present (cropped and straightened before upload) and the native-detail 1:1 patch — re-express the layer stack in the buffer's space first (`MaskAffine` + `MaskRemap`, `EditSession+MaskRemap.swift`): linear and radial masks by an exact parameter rewrite under the crop's affine, bitmap masks as a resampled raster registered per affine (`MaskRemapRasterCache`, see [caching](caching.md)).
...
`EditorWorkflowPerfTests` in `Packages/MapleCore/Tests/MapleCoreTests/Performance/` drives the production `EditSession` → `RenderActor` → `GpuLiveDriver` path on an isolated copy of the reference RAW. On macOS it mounts a visible Metal layer. It reports first CPU preview, first GPU submission acknowledgment and interactive develop readiness separately. Exposure, white-balance, contrast and point-curve drags target 60 Hz with rolling deadlines, resetting overdue schedules instead of replaying missed inputs. Actual delivered cadence and intervals are reported; this is not display-synchronized input. Each input captures its existing scheduling task; those tasks are joined after input delivery to obtain exact generations without making the input generator wait for scheduler admission. The reports include observed publication counts and latency percentiles; cancelled/coalesced requests remain visible in the counts. The same-session check calls `ensureRenderStarted()` again and verifies no new decode; it is not a view remount or image-switch timing. The separate NR-disabled arm is an attribution control, never the normal-quality acceptance result.
...
| `CloudSource`      | Maple Self Hosted / Hosted API | lists via `/api/fs/dir`, `ImageRef.id` is `fs:<absPath>`; thumbs and previews via the unified `/api/thumb                                                    | preview/:slug/\*`routes, original bytes via`/api/fs/raw` (mirror failover) |
| `ComposedSource`   | pairs two of the above         | metadata and thumbs from the API, RAW bytes and XMP writes from SMB on the LAN                                                                               |

Every cloud client holds absolute server paths (`SearchAsset.abs_path`, the `fs:<absPath>` ids above, `/api/fs/dir` listings), while the derivative routes are addressed by `slug:relPath` (#1325). `MapleCloudKit/Cloud/CloudAddressResolver.swift` bridges the two at the HTTP edge: it loads `/api/folders` once per client (one shared in-flight request across a grid's burst of thumb calls), picks the registered library whose root is the longest whole-segment ancestor of the path, percent-encodes each relative segment the way `encodeURIComponent` does, and refreshes the list at most once on a miss so a newly registered library resolves without a relaunch. `CloudThumbClient` and `CloudSource` both go through it; `/api/preview`'s `202` "indexing" reply (no catalog row yet) is surfaced as nil / a thrown 202 so the Preview screen keeps its thumbnail. Original bytes stay on `/api/fs/raw` because `/api/image/:slug/*` has no mirror read-failover (#926), and listings stay on `/api/fs/dir` because `/api/folder/:slug/*` carries no video entries, sizes, EXIF or sidecar pairing — both are server-side gaps tracked on #1325, not client choices to revisit here.
...
`FileProviderIdentifier` is the item vocabulary: assets, folders, non-indexed files, sidecars (including `(conflict from <device>).xmp` copies), a per-library trash container, and synthetic `.maple/`, `.maple/thumbs/` and `.maple/previews/` directories the extension fabricates client-side because the server hides dotdirs from `/api/fs/dir` (previews are `<filename>.avif`, fetched from `GET /api/fs/preview?path=`; both derived entries are versioned by the asset's sidecar mtime so a server-side edit refetches them). Thumbnail filenames inside them come from `MapleThumbCacheKey.sha256Prefix16(basename)` — the first 16 hex chars of SHA-256 over the _filename_, not the path, which is what lets `.maple/` travel with the photos. That helper is the single source of truth shared with `Cache/ThumbnailDiskCache.swift`, the Bun server, and the web cache.
