...
pub struct AdjustmentModel {
    ...
    /// 1D LUT (ticket #371, re-derived 2026-05-24 against the 4-tier DCP
    /// distribution per #397 § 4.2); `Look::Neutral` short-circuits the
    /// stage for pure scene-referred AgX output.
    pub look: Look,
...
mod tests {
    ...
    #[test]
    fn look_defaults_to_lut() {
        // The Look default is the empirical LUT (#371, re-derived 2026-05-24
        // against the 4-tier DCP distribution per #397). `Look::Neutral`
        // remains available for strict scene-referred output.
        let m = AdjustmentModel::default();
