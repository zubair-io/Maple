...
describe('getDb — env-change awareness (#2783 / #2819)', () => {
  // The preview-route ETag/404 flake: a test file that connects the
  // singleton under its own MAPLE_MONGO_DB and never closes it leaks that
  // connection into the next file, whose own MAPLE_MONGO_DB override is
  // then silently ignored. getDb must re-read the env pair on every call
  // and reconnect when it changed.
