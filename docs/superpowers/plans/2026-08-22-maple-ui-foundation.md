...
- Token _names_ are semantic (`radius_md`, `spacing_lg`), never literal (`radius_8`) — a future
...
- Commit generated output _with_ its source change in the same commit — never leave `Generated/`
...
part of the unsupported set). Add `Target::Xaml` to the _other_ schemas' unsupported-target arms so
...
`MapleValueChip`, `MapleToolButton`) live _below_ the token block and aren't touched by codegen —
