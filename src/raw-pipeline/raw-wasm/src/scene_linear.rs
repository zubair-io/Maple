//! Scene-linear render surface (fp16 RGBA) — split from `lib.rs` to keep it
//! under the file-size budget (#2661 pushed it past the ceiling). Same
//! precedent as `export.rs` / `preview.rs`; `lib.rs` re-exports everything so
//! `crate::`-level paths (and `use super::*` in tests) keep resolving.

use raw_core::xmp as xmp_mod;
use wasm_bindgen::prelude::*;

use crate::{as_shot_wb, cpu_budget};
