...
async function seedAsset(db: Db, libraryId: ObjectId, mapleId: string): Promise<void> {
  await db.collection('assets').insertOne({
    _id: new ObjectId(),
    fileinfo: [{ path: '', filename: `${mapleId}.dng`, library_id: libraryId, deleted_at: null }],
    size: 4,
    mtime: Date.now(),
    rating: 0,
    flag: 0,
...
describe('POST /api/libraries/:libraryId/backup/exists', () => {
  ...
  it('scopes presence to the requested library', async () => {
    ...
    const res = await post(libraryId.toHexString(), { maple_ids: ['shared-id'] });
    expect(res.status).toBe(200);
    const body = (await res.json()) as { missing: string[] };
  ...
  it('unknown library → 404', async () => {
    if (!mongo || !db) return;
    const res = await post(new ObjectId().toHexString(), { maple_ids: ['anything'] });
