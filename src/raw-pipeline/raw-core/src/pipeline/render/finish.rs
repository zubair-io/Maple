...
//! then manual geometry, then crop / straighten.
//!
...
use crate::{
    ...
    stages::perspective::{self, Perspective},
    types::Crop,
};

/// Orient into display framing, apply the manual-geometry homography, then
/// apply the crop.
///
...
/// manual-geometry warp (#3410) runs second because its sliders are authored
/// against what the photographer sees, not against sensor framing. The crop
/// runs last because the crop rect is how the user frames out the transparent
/// surround a keystone leaves behind — it has to be able to see that surround.
///
/// Each of the three is skipped entirely when it is identity, so the
/// parity-harness baseline pays no extra allocation and renders bit-identically
/// to a build without any of them. Invalid rects are treated as identity.
///
/// `samples` is taken by value, not borrowed, so the fully-identity case
/// (`Normal` orientation, identity geometry, identity crop — every caller here
/// already owns a freshly quantized buffer it doesn't need afterward) can
/// return it straight back with no clone and no allocation at all, and a
/// `Normal` + real crop can crop directly out of the caller's own buffer
/// instead of cloning it first. Without this, `apply_orientation`'s `Normal`
/// arm still pays a full-buffer `to_vec()` (~27 ms at 100 MP,
...
pub(super) fn apply_geometry<T: Sample>(
    ...
    let warped = if geometry.is_identity() {
        ...
        // `apply_int_rgb` returns `None` for a transform that resolves to the
        // identity anyway (a degenerate matrix from a corrupt sidecar), which
        // lets the caller's buffer move on rather than be copied.
        match stage("perspective", || {
            perspective::apply_int_rgb(&oriented, w, h, geometry)
        }) {
            Some(warped) => warped,
            None => oriented,
        }
    ...
    stage("crop", || crop::apply_int_rgb(&warped, w, h, crop_rect))
}
...
mod tests {
    ...
    fn both_depths_permute_identically() {
        ...
        let (w16, h16, out16) = apply_geometry(
            sixteen,
            2,
            1,
            ExifOrientation::Rotate180,
    ...
    fn normal_orientation_with_real_crop_matches_cropping_directly() {
        ...
        let (gw, gh, geometry_out) = apply_geometry(
            src.clone(),
            w,
            h,
            ExifOrientation::Normal,
