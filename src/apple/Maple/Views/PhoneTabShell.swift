...
// (Library / Search / Settings) so each tab preserves its own
// push depth and the bottom tab bar stays put while drilling down.
//
// #2886: Map is NOT a tab here. #2878 originally gave it its own footer
// tab with an independent `AppShellCenterColumn` call site (`mapPath`,
// `pushMapPreview`), to dodge a visual-priority problem: `AppShellCenterColumn`'s
// map branch takes precedence over the grid, so threading `mapVM` into the
// Library tab's call site would have permanently hijacked that tab for as
// long as `librarySelection == .map`. That's actually the right behaviour
// once Map moves to the side navigation (matching Mac/iPad and the web
// app) rather than the footer tab bar — selecting MAP from the sidebar
// takes over the Library tab's content exactly as selecting Timeline or a
// folder already does. The sidebar's MAP row (`AppShellSidebar.onSelectMap`
// → `AppShell.openMap()`) is reachable only via the Library tab's drawer
// (the hamburger lives in `AppShellIPhoneShell`'s toolbar), so threading
// `mapVM`/`mapThumbClient`/`mapThumbCache`/`onSelectMapPlace` into the
// Library tab's `AppShellCenterColumn` call site (via `PhoneLibraryView` →
...
struct PhoneTabShell<SidebarContent: View, ToolbarContentT: ToolbarContent>: View {
    ...
    /// same stack a grid tap uses. A Map pin/cluster result tap also pushes
    /// here (#2886) — Map renders inside this tab's `AppShellCenterColumn`
    /// call site now, so it shares this stack rather than owning its own.
    ...
    /// current (#2886) — forwarded straight to the Library tab's
    /// `AppShellCenterColumn` call site. Populated by AppShell's
    /// `openMap()`, same three values `AppShellMacLayout` gets from the
    /// Mac/iPad sidebar's MAP row.
    let mapVM: MapViewModel?
    let mapThumbClient: CloudThumbClient?
    let mapThumbCache: CloudThumbCache?
    /// Why `.map` has no `mapVM` yet (#2848) — forwarded to the Library
    /// tab's `AppShellCenterColumn` so it renders `MapEmptyState` (no cloud
