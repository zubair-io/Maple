...
// the PNG.
//
// This is the MapleCoreTests copy of the helper originally written for
// the XCUITest harness (`MapleUITests/Helpers/GoldenStore.swift`). The
// two copies share an on-disk goldens directory at
// `src/apple/MapleUITests/Goldens/` so a baseline written by either
// harness is honoured by the other. Resolution order:
//   1. `MAPLE_VISUAL_GOLDENS_ROOT` env var (explicit override)
//   2. `MAPLE_UITEST_GOLDENS_ROOT` env var (legacy XCUITest var name)
...
enum GoldenStore {
    ...
    /// `MAPLE_VISUAL_GOLDENS_ROOT` (preferred) or `MAPLE_UITEST_GOLDENS_ROOT`
    /// (legacy XCUITest name); otherwise walk from `#filePath` up to the
    /// repo root and into `src/apple/MapleUITests/Goldens` so the unit
    /// test reuses the XCUITest harness's golden directory.
    static func goldensRoot() throws -> URL {
        if let root = ProcessInfo.processInfo.environment["MAPLE_VISUAL_GOLDENS_ROOT"],
           !root.isEmpty {
        ...
        // Walk from #filePath:
        //   .../Tests/MapleCoreTests/Helpers/GoldenStore.swift
        //   → repoRoot()/src/apple/MapleUITests/Goldens
        let here = URL(fileURLWithPath: #filePath)
            ...
            .appendingPathComponent("src/apple/MapleUITests/Goldens")
            .standardizedFileURL
        if FileManager.default.fileExists(atPath: here.path) {
            return here
        }
        // CWD-relative fallbacks (XCUITest legacy).
        ...
        // Fall back to the #filePath-derived path even if it doesn't exist
        // yet — the caller will create it on first-run baseline write.
        return here
    }
