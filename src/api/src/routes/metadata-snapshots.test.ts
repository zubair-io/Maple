...
import {
  metadataSnapshotsRoutes,
  overrideToXmpSnapshot,
} from "./metadata-snapshots.ts";
...
beforeEach(async () => {
  tmpDir = await fs.realpath(
    await fs.mkdtemp(path.join(os.tmpdir(), "meta-snap-test-")),
  );
...
describe("overrideToXmpSnapshot", () => {
  ...
  test("returns empty object when override and exif are null", () => {
    expect(
      overrideToXmpSnapshot({ exif: null, metadata_override: null }),
    ).toEqual({});
  });
...
describe("POST /api/metadata/snapshots — real DB", () => {
  ...
  test("returns snapshots in request-path order (not DB result order)", async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.snapshots.map((s: { path: string }) => s.path)).toEqual(
      requestPaths,
    );
  });
  ...
  test("does not leak data from same-named file in a different library", async () => {
    ...
    await db!.collection("assets").insertOne({
      fileinfo: [
        { path: "", filename, library_id: otherFolderId, deleted_at: null },
      ],
      size: 1,
      mtime: 1,
      rating: 0,
