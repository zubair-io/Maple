...
- **Apple**: on the default wgpu path (#1066), the f32 scene-linear buffer is uploaded to the GPU and the full view transform (AgX, split-tone, grain, display encode) plus sharpen/NR run as WGSL compute shaders, presenting via wgpu → CAMetalLayer. The CPU/Metal fallback runs the full Rust FFI chain (AgX + split-tone + grain + display encode) and Metal kernels for sharpen/NR. There is no CIImage filter chain — the color math is the Rust core.
