 ...
 *
 * The job re-reads its config from `worker_config` on every pass, so a PUT
...
import { prunedThroughCursor } from '../db/changes.repo.ts';
...
export const changeLogGcRoutes = new Elysia()
  .get('/api/change-log-gc/status', async () => ({
    config: await loadChangeLogGcConfig(),
    rows: await estimatedRows(),
    pruned_through: await prunedThroughCursor().catch(() => 0),
  }))
  .put(
    '/api/change-log-gc/config',
    async ({ body }) => ({ ok: true, config: await saveChangeLogGcConfig(body) }),
    {
      body: t.Object({
