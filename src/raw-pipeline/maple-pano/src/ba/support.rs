...
/// Stage-D cap on prune-and-resolve iterations per gate round. The loop
/// exits as soon as a pass prunes nothing, so this is a fixed-point
/// bound, not a budget: each re-solve can nudge a handful of residuals
/// across the threshold (measured on pano_01 at full-res scale: a
/// passing-mean frame dying on a single 6.005 px block after a 2-round
/// cap), and the cumulative [`PRUNE_MAX_FRACTION`] guard — not the
/// round count — is what stops a bad frame from cleaning itself.
pub(super) const PRUNE_ROUNDS_MAX: usize = 8;
/// Per-frame guard, cumulative across the prune rounds of one gate
/// round: when pruning would shed more than this fraction of a frame's
/// ORIGINAL correspondences, the frame is misaligned rather than
/// polluted — NONE of its correspondences are pruned (in that round),
/// so the §5.3 gate judges it on its real residuals instead of a
/// self-cleaned set.
const PRUNE_MAX_FRACTION: f64 = 0.3;

/// Correspondence-pair count per local frame (each pair touches both
/// endpoint frames) — the denominator of the cumulative
/// [`PRUNE_MAX_FRACTION`] guard.
pub(super) fn pairs_per_frame(blocks: &[Block], n_local: usize) -> Vec<usize> {
    debug_assert_eq!(blocks.len() % 2, 0, "blocks come in directed pairs");
    let mut pairs_of = vec![0usize; n_local];
    for pair in blocks.chunks_exact(2) {
        pairs_of[pair[0].src] += 1;
        pairs_of[pair[0].dst] += 1;
    }
    pairs_of
}
...
/// their directed blocks, honoring the cumulative per-frame
/// [`PRUNE_MAX_FRACTION`] guard on both endpoint frames
/// (`original_pairs` = the gate round's starting support per frame,
/// `pruned_pairs` = pairs shed so far, updated here).
///
...
pub(super) fn prune_outlier_blocks(
    ...
    // Frames whose cumulative shed would exceed the guard keep
    // everything (all-or-nothing per round — no partial self-cleaning).
    let guarded: Vec<bool> = (0..n_local)
        .map(|f| {
            original_pairs[f] > 0
                && ((pruned_pairs[f] + candidates_of[f]) as f64)
                    > PRUNE_MAX_FRACTION * (original_pairs[f] as f64)
        })
        .collect();
    ...
    for pair in 0..n_pairs {
        ...
        if keep {
            ...
            pruned += 1;
            pruned_pairs[fwd.src] += 1;
            pruned_pairs[fwd.dst] += 1;
        }
    }
    if pruned == 0 {
        return None;
