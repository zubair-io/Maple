...
All native calls go through `Native/RawFfi.cs`: a `static unsafe class` of `[DllImport("raw_ffi.dll", CallingConvention = Cdecl)]` externs. Return convention is `0 = success`, with the message read from `maple_last_error()` on the **same thread** as the failing call. Struct layouts are hand-mirrored in `Native/MapleAdjustmentParams.cs` and `Native/RawFfiStructs.cs`, field-for-field against `raw-ffi/src/scene_linear_chain.rs`, append-only at the tail. `RawFfi.VerifyAbi()` asserts `sizeof(MapleAdjustmentParams) == 672` at startup so a drifted mirror fails loudly instead of corrupting pixels.
...
`tools/check-file-budget.sh` — the 400-line soft / 600-line hard file-size budget — scans `*.rs *.swift *.ts *.tsx *.js *.py` only. `.cs` and `.xaml` are outside its extension list in both the explicit-paths branch and the repo-wide `find`, so no Windows C# file is measured, allowlisted, or blocked by it. Several already exceed the hard limit (`MainWindow.xaml.cs` at 839 lines, `CloudClient.cs` at 631, `EditSessionViewModel.Library.cs` at 605). The splitting discipline visible throughout the codebase — the `MainWindow.*.cs` partials, the `EditSessionViewModel` partials, the `*Math`/`*Logic` extractions — is convention, not enforcement.

Likewise, `lefthook.yml`'s Prettier hook globs `ts,tsx,js,jsx,html,scss,css,json,yaml,yml,md,graphql`, so there is no formatting gate on C# or XAML either.
