...
use crate::buffers::{MapleSceneLinearBuffer, MapleSceneLinearBufferF32};
...
// ─── f32 variants (#482) ─────────────────────────────────────────────────
//
// Mirror the four fp16 entries above (file/bytes × full/sized) returning
// `MapleSceneLinearBufferF32` so consumers can hold the scene-linear
// buffer at full f32 precision end-to-end (#416). The fp16 entries are
// kept intact — Apple still consumes them today; the per-tick FFI chain
// is fp16 in/out, so migrating Apple's render path alone would silently
// round-trip back to fp16 every slider tick. Apple migration is tracked
// as a separate ticket once `maple_apply_scene_linear_chain` grows an
// f32 sibling. Web consumes f32 directly (RGBA32F FBOs).
