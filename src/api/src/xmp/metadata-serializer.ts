...
function bagBlock(qname: string, values: string[]): string {
  const lis = values
    .map((v) => `    <rdf:li>${escapeText(v)}</rdf:li>`)
    .join("\n");
  return [
    `  <${qname}>`,
    "   <rdf:Bag>",
...
function buildMetadataAttrParts(m: XmpMetadataInput): string[] {
  ...
  if (m.gpsLatitude != null)
    push("exif:GPSLatitude", gpsToXmp(m.gpsLatitude, "lat"));
  if (m.gpsLongitude != null)
    push("exif:GPSLongitude", gpsToXmp(m.gpsLongitude, "lon"));
  ...
  else if (m.copyrightStatus === "public-domain")
    push("xmpRights:Marked", "False");
...
function buildMetadataNestedBlocks(m: XmpMetadataInput): string[] {
  ...
  if (m.copyrightNotice)
    blocks.push(langAltBlock("dc:rights", m.copyrightNotice));
  if (m.usageTerms)
    blocks.push(langAltBlock("xmpRights:UsageTerms", m.usageTerms));
  if (m.keywords && m.keywords.length > 0)
    blocks.push(bagBlock("dc:subject", m.keywords));
  return blocks;
}
...
function requiredNamespacePrefixes(m: XmpMetadataInput): Set<string> {
  ...
  if (m.copyrightStatus && m.copyrightStatus !== "unknown")
    used.add("xmpRights");
  if (m.keywords && m.keywords.length > 0) used.add("dc");
  if (m.timeZone) used.add("papp");
  if (m.rating != null && m.rating > 0) used.add("xmp");
  if (
    (m.flag && m.flag !== "unflagged") ||
    m.colorLabel ||
    (m.isScreenshot !== undefined && m.isScreenshot !== null)
  )
    used.add("papp");
  return used;
}
  ...
  if (newAttrs.length > 0 || nsDecls.length > 0) {
    ...
    if (descTagEnd !== -1) {
      ...
      const toInsert =
        ...
        (newAttrs.length > 0 ? "\n   " + newAttrs.join("\n   ") : "");
      result =
        result.slice(0, insertPoint) + toInsert + result.slice(insertPoint);
    }
  }
