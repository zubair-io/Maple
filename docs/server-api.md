...
**Legacy.** The older absolute-path surface. Since #1325 the web and Apple clients only call `/api/fs/roots` and `/api/fs/list` (pre-registration folder pickers — the web library picker, the web and Apple Imports source pickers — which walk the jail before a library, and so a `slug:relPath` address, exists), plus the two routes below that have no unified equivalent yet: `/api/fs/dir` (Apple File Provider and cloud-source browse — the enriched listing with video entries, size/ext/mtime, EXIF, paired sidecars and cursor paging that `/api/folder/:slug/*` does not carry) and `/api/fs/raw` (Apple `CloudSource` — the only original-bytes route carrying #926's mirror read-failover, which `/api/image/:slug/*` lacks). The Windows cloud client still consumes its own set. Each route's remaining consumers are listed in its row; a route with none is retired server-side in a follow-up. All share the `MAPLE_ROOTS` jail and the system-directory denylist.

| Method | Path               | Auth   | Purpose                                                                                                                                                                                                                                                                   |
| ------ | ------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/fs/roots`    | +file  | The configured browse roots                                                                                                                                                                                                                                               |
| GET    | `/api/fs/list`     | +file  | Subdirectories under `?path=` (no files). Backs the web library picker and the web + Apple Imports source pickers. `showAll=0\|1`                                                                                                                                         |
| GET    | `/api/fs/dir`      | +file  | Directories and image files at one level, enriched with catalog `asset_id`, EXIF, and paired sidecars. Two Mongo `$in` lookups per request. Paged via `cursor` + `limit`. Apple File Provider + cloud browse, Windows                                                     |
| GET    | `/api/fs/dir-fast` | +file  | Same paging contract, pure filesystem — no Mongo, no EXIF, no sidecars. No remaining consumer (web and Apple pickers moved to `/api/fs/list`); the grids list via `/api/folder`                                                                                           |
| GET    | `/api/fs/raw`      | +file  | Stream the original bytes at `?path=`. Allowlisted to RAW ∪ bitmap extensions; others get 415. May be served from a mirror when the primary volume is unreachable (#926) — the reason Apple `CloudSource` still reads here rather than `/api/image`. Apple + Windows only |
| GET    | `/api/fs/thumb`    | bearer | Thumbnail AVIF for `?path=`, cached at `.maple/thumbs/`. A cache hit is one `readFile`. Deliberately no `size` parameter. No remaining consumer (web and Apple fetch `/api/thumb`)                                                                                        |
| GET    | `/api/fs/preview`  | bearer | 1280 px preview for `?path=`, cached at `.maple/previews/`. Falls back to on-demand generation for un-indexed files. No remaining consumer (web and Apple fetch `/api/preview`)                                                                                           |
