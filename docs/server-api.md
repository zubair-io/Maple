...
**Legacy.** The older absolute-path surface. Since #1325 every client fetches thumbnails and previews by address, so `/api/fs/thumb`, `/api/fs/preview` and `/api/fs/dir-fast` have no remaining consumer and are retired server-side in a follow-up. What is still called here: `/api/fs/roots` and `/api/fs/list` (pre-registration folder pickers — the web library picker and the web, Apple and Windows Imports source pickers — which walk the jail before a library, and so a `slug:relPath` address, exists), plus the two routes with no unified equivalent yet. Two server gaps hold the native clients on those: `/api/folder/:slug/*` reports no size, mtime or EXIF and no absolute path (the Apple File Provider and the Windows sync root stamp placeholders with size and mtime; the Windows grid needs the enriched listing), and `/api/image/:slug/*` carries no #926 mirror read-failover (the reason Apple `CloudSource` and the Windows download-to-edit and hydration paths still read `/api/fs/raw`). Each route's remaining consumers are listed in its row. All share the `MAPLE_ROOTS` jail and the system-directory denylist.
