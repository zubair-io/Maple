...
use raw_core::{
    decode::decode_bytes,
    pipeline::{
        fit_auto_profile_from_raw, fit_profile_curve_from_raw, render_from_raw_with_quality,
...
pub unsafe extern "C" fn maple_compute_auto_profile_lut(
    ...
    with_large_stack(move || {
        ...
        let ext = raw_path.extension().and_then(|e| e.to_str()).unwrap_or("");
        let raw_img =
            match raw_core::pipeline::stage("ffi_auto_lut_decode", || decode_bytes(&raw_bytes, ext))
            {
                Ok(r) => r,
                Err(e) => {
