  ...
  public static func apply(
    ...
    for asset in targets {
      ...
      if let session = sessions[asset.id] {
        ...
        session.endEdit()
        do {
          try await session.sidecarStore?.writeConfirmed(
            model: session.model, culling: session.culling)
          written += 1
        } catch { continue }
        continue
      }
      ...
      do {
        try await store.writeConfirmed(model: mergedModel, culling: culling)
        written += 1
      } catch { continue }
    }
    return written
