...
public struct MuiPageEditor: View {
  ...
  public var body: some View {
    let vm = buildMuiPageEditorVM(photos: photos, activePhotoId: activePhotoId)
    MuiSplitLayout {
      MuiToolDock(
        entries: Self.toolEntries, activeId: $activeToolId, toolSelected: handleToolSelected
      )
      ...
      VStack(spacing: 0) {
        ...
        MuiImageCanvas(
          url: vm.photoURL, cropMode: Self.cropModeActive(toolId: activeToolId), cropRect: $cropRect
        )
        ...
        if let controlTabId = Self.armedControlTab(toolId: activeToolId) {
          ...
          MuiControlSurface(
            ...
            sliderChanged: { id, value in
              values = Self.applySliderChange(values, id: id, value: value)
            }
          )
      ...
      MuiInspectorPanel(
        title: "Adjustments",
        tabs: [MuiTab(id: "adjust", label: "Adjust"), MuiTab(id: "info", label: "Info")],
        showBack: false, activeTabId: $inspectorTabId
      ) {
        if inspectorTabId == "adjust" {
          MuiAdjustmentsPanel(
            ...
            valueChanged: { id, value in
              values = Self.applySliderChange(values, id: id, value: value)
            }
          )
        } else {
          infoTab(title: vm.infoTitle)
        }
      }
    }
  ...
  private func infoTab(title: String) -> some View {
    VStack(alignment: .leading, spacing: MuiTokens.spacingXs) {
      MuiText(title, variant: .rowLabel)
  ...
  public static func applySliderChange(_ values: [String: Double], id: String, value: Double)
    -> [String: Double]
  {
    var next = values
    next[id] = value
    ...
    MuiAdjustmentTab(
      ...
      groups: [
        MuiAdjustmentGroup(
          ...
          sliders: [
            MuiAdjustmentSlider(
              id: "temp", label: "Temp", min: 2000, max: 10000, step: 50, unit: "K"),
