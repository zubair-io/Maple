// EditSession+Decode.swift — Rust FFI decode + decoded-image cache lifecycle.
//
...
//   • `parseSidecarModel` / `sidecarMtime` / `decodedCacheIsFreshForCurrentAsset`
//     — sidecar bookkeeping that gates cache reuse
//   • `renderForExport`      — full-quality export bypass of the preview cache
//   • Test hooks for the cache state machine
//
// The actual stored cache fields (`decodedImage`, `decodeTask`,
// `refineDecodeTasks`, `decodedAtModel`, `decodedSidecarMtime`,
// `decodedRawResolution`, `decodedForAssetID`) live on `EditSession` —
...
extension EditSession {
    ...
    // MARK: - Decoded-image cache lifecycle
