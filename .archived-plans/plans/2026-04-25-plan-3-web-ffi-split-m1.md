...
> **Brief:** [`.archived-plans/specs/2026-04-25-plan-3-web-ffi-split-brief.md`](../specs/2026-04-25-plan-3-web-ffi-split-brief.md). This plan implements the brief's § 10 "Recommended cut": **M1 only.**
>
...
Append to `.archived-plans/plans/2026-04-25-plan-3-web-ffi-split-m1.md` (this very file) a closing block under a new heading `## M1 verification log` with the following content (replace `<TIMESTAMP>` with `date -u +%FT%TZ` output and `<HASH>` with `git log -1 --format=%h`):
...
```bash
git add .archived-plans/plans/2026-04-25-plan-3-web-ffi-split-m1.md
git commit -m "$(cat <<'EOF'
...
- [ ] Brief reference: `.archived-plans/specs/2026-04-25-plan-3-web-ffi-split-brief.md` — exists in the main checkout (verified during plan authoring).
- [ ] Plan 1 v2 reference: `.archived-plans/plans/2026-04-24-ffi-split-plan-1.md` — exists.
- [ ] Plan 2 reference: `.archived-plans/plans/2026-04-25-plan-2-dev-chain-metal-kernels.md` — exists.
- [ ] Deep Zoom plan reference: `.archived-plans/plans/2026-04-25-deep-zoom-tile-rendering.md` — exists.
