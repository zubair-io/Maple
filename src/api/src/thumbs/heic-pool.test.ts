...
describe('HEIC thumbnail offload', () => {
  ...
  it('produces bytes identical to the pre-offload inline chain', async () => {
    ...
    const ok = await renderImageThumbToFile(FIXTURE_HEIC, viaPool, SIZE_PX, 'heic');
    expect(ok).toBe(true);
  ...
  // Proves the decode actually left the main thread. The pool reports which
  // path ran via `viaWorker`: a `decodeHeic` reply only ever originates from
  // the spawned worker (the manager sets `viaWorker: true` in its message
  // handler), while the in-process fallback sets `viaWorker: false`. Asserting
  // `viaWorker === true` catches the exact regression a timing check cannot —
  // a broken worker URL / spawn that silently degrades to blocking in-process
  // ALSO can't settle within a microtask drain (it starts with async
  // `readFile`), so the old microtask-only assertion would have passed it.
  // Going through `decodeHeicThumbOffThread` directly (not `renderImageThumbToFile`)
  // keeps the telemetry observable instead of swallowed behind the boolean.
  it('runs genuinely off-thread (decode reports viaWorker)', async () => {
    if (!(await fixturePresent())) return; // fixture missing, soft pass

    const out = path.join(dir, 'off-thread.jpg');
    const result = await decodeHeicThumbOffThread(FIXTURE_HEIC, out, SIZE_PX);
    expect(result.ok).toBe(true);
    // The load-bearing assertion: the work ran on the Worker, not the
    // in-process fallback. Fails if the worker can't spawn.
    expect(result.viaWorker).toBe(true);
  });
  ...
  it('leaves non-HEIC formats on the inline sharp path (unaffected)', async () => {
    ...
    expect(Math.max(meta.width ?? 0, meta.height ?? 0)).toBeLessThanOrEqual(SIZE_PX);
  });
});
