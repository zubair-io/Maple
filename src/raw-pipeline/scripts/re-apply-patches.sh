...
#
# SCOPE: this script patches crates under vendor/ AND the hand-maintained
# `[patch.crates-io]` copies under third_party/, which `cargo vendor` does not
# regenerate (those crates are no longer emitted into vendor/ at all):
#
#   * vendor/ort-sys/build.rs            — patch 1 below
#   * third_party/zune-jpeg/src/mcu.rs   — patch 2 below (#3596)
#   * third_party/rav1d/                 — NOT handled here. Its patch
#     (patches/rav1d-1.1.0-c-unwind.patch) predates this script and is applied
#     by hand; see third_party/rav1d/MAPLE-PATCH.md.
#
...
# ---------------------------------------------------------------------------
# Patch 2: zune-jpeg non-interleaved multi-scan baseline decode  (#3596)
# ---------------------------------------------------------------------------
# Applied to third_party/zune-jpeg/, the `[patch.crates-io]` copy — NOT to
# vendor/, which no longer contains the crate. Idempotent: the applier is a
# no-op when the marker comment is already present, and fails loudly when a
# hunk's context has moved (i.e. zune-jpeg was upgraded without adapting
# patches/zune-jpeg-0.5.15-noninterleaved-scan.patch).
ZUNE_DIR="$RAW_PIPELINE_DIR/third_party/zune-jpeg"

echo ""
echo "==> zune-jpeg non-interleaved multi-scan decode (#3596)"

if [[ ! -f "$ZUNE_DIR/src/mcu.rs" ]]; then
    echo "ERROR: $ZUNE_DIR/src/mcu.rs not found — is third_party/ populated?" >&2
    exit 1
fi

python3 "$SCRIPT_DIR/apply-zune-jpeg-patch.py" "$ZUNE_DIR"

echo ""
echo "==> All patches applied."
echo "    Commit vendor/ort-sys/build.rs, vendor/ort-sys/.cargo-checksum.json"
echo "    and third_party/zune-jpeg/src/mcu.rs."
