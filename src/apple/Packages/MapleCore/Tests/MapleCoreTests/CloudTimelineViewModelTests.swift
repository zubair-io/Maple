...
final class CloudTimelineViewModelTests: XCTestCase {
  ...
  // MARK: - Union of PhotoKit + cloud buckets (instant / offline first paint)
  ...
  func test_init_seedsBucketsFromPhotoKitWithoutNetwork() throws {
    let server = URL(string: "https://example.test")!
    // An unreachable client — if init touched the network this would never
    // resolve synchronously anyway; the assert runs before any await.
  ...
  func test_loadPage_rendersLocalCellsWhenCloudPageEmpty() async throws {
    ...
    let key = CloudTimelineViewModel.BucketKey(year: 2024, month: 7)
    let merged = vm.mergedPagesByBucket[key] ?? []
    XCTAssertEqual(merged.count, 2, "expected the two local cells to survive an empty cloud page, got \(merged)")
