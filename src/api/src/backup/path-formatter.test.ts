...
describe("sanitizeLocationSegments", () => {
  ...
  test("trims and drops empties", () =>
    expect(sanitizeLocationSegments([" A ", "", "  ", "B"])).toEqual([
      "A",
      "B",
    ]));
  ...
  test("drops path-traversal tokens (., .., leading dot) per segment", () =>
    expect(
      sanitizeLocationSegments([".", "..", ".hidden", "...dotty", "Keep"]),
    ).toEqual(["Keep"]));
  test("drops null/undefined elements in the array", () =>
    // @ts-expect-error testing invalid input types
    expect(sanitizeLocationSegments(["A", null, "B", undefined, "C"])).toEqual([
      "A",
      "B",
      "C",
    ]));
...
describe("isSafeFilename", () => {
  ...
  test("name over 255 chars → false", () =>
    expect(isSafeFilename("a".repeat(256))).toBe(false));
  test("name with forward slash → false", () =>
    expect(isSafeFilename("foo/bar.jpg")).toBe(false));
  test("name with backslash → false", () =>
    expect(isSafeFilename("foo\\bar.jpg")).toBe(false));
  test("'.' → false", () => expect(isSafeFilename(".")).toBe(false));
  test("'..' → false", () => expect(isSafeFilename("..")).toBe(false));
  test("leading dot → false", () =>
    expect(isSafeFilename(".hidden")).toBe(false));
  test("normal filename → true", () =>
    expect(isSafeFilename("IMG_0420.HEIC")).toBe(true));
  test("exactly 255 chars → true", () =>
...
describe("formatBackupPath — filename safety", () => {
  ...
  test("empty filename → throws", () => {
    expect(() =>
      formatBackupPath({ captureDate: capture, location: null, filename: "" }),
    ).toThrow("unsafe filename");
  });
