...
Seams are **Voronoi by source-border distance**: each covered canvas pixel belongs to the frame whose projection of it sits deepest inside that frame. Deterministic and content-blind. `blend.rs` is Burt–Adelson multi-band Laplacian blending with the separable `[1,4,6,4,1]` binomial kernel, validity-weighted at every pyramid level so masked regions never bleed value in; band count is `log2(minimum overlap width)`, capped.

Compositing always runs the **tiled** path on the rotation strategy. `composite_tiled` takes already-solved gains and a strip height, decodes each source frame on demand one at a time, warps it into a `canvas_width × tile_rows` strip, and frees the pixels before the next frame. The ownership mask needs only camera geometry, no pixels. Without this, all N full-resolution frames are resident at once.
