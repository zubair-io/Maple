...
//
// The enumerators that list a library's real contents: root, folder, trash,
// plus the shared page codec and error mapping they all use. The synthetic
// `.maple/` derivative views live next door in `MapleDerivedEnumerators.swift`.
    ...
    public func enumerateChanges(for observer: NSFileProviderChangeObserver,
        ...
        Task { [weak self] in
            ...
            do {
                ...
                if e.current > 0, let changeCursor {
                    changeCursor.store.save(e.current, domain: changeCursor.domainID)
                }
                log.notice("folder stale cursor (server current=\(e.current)); advancing anchor and requesting full re-enumeration")
