...
5. **The 67 px stencil is non-negotiable.** This plan does NOT introduce any tile path for dehaze; the existing whole-image fallback at the FFI level (`pipeline.rs:543-545`) and the UI clamp (`docs/superpowers/plans/2026-04-25-deep-zoom-tile-rendering.md` Architecture point 3) carry the load. The Apple `applySceneDehaze` wrapper computes over the whole CIImage extent — same shape as `applySceneClarity` and `applySceneNRColor`, except the working-buffer high-water-mark is higher (8 single-channel scratches + 2 RGBA scratches = ~14 MB at 6K×4K, well under the iPhone 200 MB tile cap).
