 ...
 * restricts raw filesystem imports — the same workaround the sibling
 * benchmarks use, and for the same reason.
 */
...
async function measureMongo(assetCount: number, stages: string[]): Promise<MongoRows> {
  ...
  try {
    ...
    const db: Db = client.db(`maple_stageclaim_${Date.now()}`);
    ...
    const deadCount = await timed(() => assets.countDocuments({ [`stages.${STAGE}.dead`]: true }));
