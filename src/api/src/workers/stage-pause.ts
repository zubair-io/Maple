 ...
 *
 * The operator's pause button writes the worker's `paused` flag and nothing
 ...
 * straight back into the same failure. The reason lives on the same
 * `worker_config` row (`pause_reason`), so it survives restarts and is read by
