...
const TEST_DB = withTestDb(`maple_test_dead_letter_${process.pid}`);
...
beforeAll(async () => {
  ...
  if (!mongoReachable) {
    console.log('[indexer-dead-letter.test] skipping: MongoDB unreachable at', MONGO_URI);
    return;
  }
  db = mongo!.db(TEST_DB);
...
async function insertAll(rows: DeadLetterDoc[]): Promise<void> {
  await db!.collection<DeadLetterDoc>(COLL).insertMany(rows as unknown as DeadLetterDoc[]);
}
...
describe('groupDeadLetterByError', () => {
  it('clusters identical errors and orders by count desc', async () => {
    if (!mongoReachable) return;
    const { groupDeadLetterByError } = await import('../src/indexer/indexer.repo.ts');
    await insertAll([
      seed({
        key: 'k1',
        stage: 'ai',
...
describe('resetDeadLetter', () => {
  it('deletes by key and returns deletedCount', async () => {
    ...
    const remaining = await db!.collection(COLL).find({}).toArray();
  ...
  it('deletes by stage and returns deletedCount', async () => {
    ...
    const survivors = await db!.collection(COLL).find({}).toArray();
  ...
  it('narrows when both filters are provided', async () => {
    ...
    const remaining = await db!.collection(COLL).find({}).sort({ key: 1, stage: 1 }).toArray();
    expect(remaining).toHaveLength(2);
    expect(remaining.map((r) => `${r.key}/${r.stage}`)).toEqual(['asset-1/exif', 'asset-2/ai']);
  });
  ...
  it('rejects calls with neither filter (refuses to wipe everything)', async () => {
    ...
    await expect(resetDeadLetter({})).rejects.toThrow(/must provide at least one/i);
    const survivors = await db!.collection(COLL).find({}).toArray();
    expect(survivors).toHaveLength(1);
  });
