...
public actor RenderedPreviewCache {
  ...
  public func preview(for assetURL: URL, screenWidth: Int) -> CIImage? {
    ...
    if let (img, _) = memCache[key] {
      cacheLog.notice(
        "RenderedPreviewCache: MEMORY HIT \(assetURL.lastPathComponent, privacy: .public) v\(self.viewTransformVersion) — SKIPPING Rust pipeline"
      )
      return img
    }
    // Disk
    ...
    guard fm.fileExists(atPath: fileURL.path),
      ...
      cacheLog.notice(
        "RenderedPreviewCache: MISS \(assetURL.lastPathComponent, privacy: .public) v\(self.viewTransformVersion) — pipeline will run"
      )
      return nil
    }
    cacheLog.notice(
      "RenderedPreviewCache: DISK HIT \(assetURL.lastPathComponent, privacy: .public) v\(self.viewTransformVersion) (\(data.count) bytes) — SKIPPING Rust pipeline"
    )
    evictIfNeeded()
  ...
  /// A queued GPU exit write owns both its file revision and destination.
  /// Neither may be re-derived after readback while another folder is opening.
  ...
  func storePreview(_ image: CIImage, for snapshot: WriteSnapshot) {
    ...
    guard let dir = snapshot.directory else { return }
    let fileURL = dir.appendingPathComponent("\(key).jpg")
    let ctx = CIContext()
    guard
      let data = ctx.jpegRepresentation(
        of: image,
