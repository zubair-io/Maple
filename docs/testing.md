...
| `apple.yml`           | `swift-build`, `swift-regressions`                                                           | MapleCore compile gate plus executable scheduling/cache/geometry/Metal regressions against a real host Rust archive                                                                                                                 | pushes/PRs touching `src/apple/**` or `src/raw-pipeline/**`                          |
...
**Web asynchronous display scopes (#3397)** add `raw-gpu/src/scope_sample/tests.rs` to the ordinary GPU test suite: sampled pixels agree with the presentation quantizer, sampling leaves the source unchanged, in-flight maps retain their frozen pixels, and teardown/cancellation/error paths release or retire the staging buffer safely. Angular tests exercise coalescing, trailing captures, pending edits, monotonic delivery and session replacement.

After the canonical GPU+parallel WASM build and `ng build maple-syrup --configuration production`, run `bun run check:live-scopes /absolute/path/to/100mp.dng dist/maple-syrup/browser srgb` and repeat with `display-p3`. This gate requires an actual Chrome WebGPU adapter and explicit RAW; it never skip-passes a missing fixture. It delays JavaScript delivery of an already completed native scope map while actual worker renders complete, releases it and checks the final sample against the same frame's independently captured canvas pixels. Separate queue-completion holds prove cold open and edits retain admission until GPU completion, with at most one frame pending; an injected completion rejection must report failure and recover. Native queue completion can include earlier native maps, so delayed JavaScript delivery does not prove native map independence. The canvas oracle runs only in an untimed correctness frame before the render yields to compositor commit; timed drags perform no canvas readback. Exposure must change real pixels, and a streamed SHA-256 check confirms the original RAW is unchanged.

The script reports 30 unpaced and 30 paced full-XMP Neutral render replies, GPU-completed render timing, asynchronous map timing, final-scope latency and GPU allocation deltas after warmup. It also drives the actual product scheduler with 30 independent 60Hz inputs and reports coalescing plus the final reply/scope age from the last input. Every paced reply must remain within the 50ms hard limit, with 16ms the product target; the final scope must arrive within 50ms after that frame's reply, and every tracked GPU resource allocation count must remain unchanged. These are GPU-completion acknowledgements and scope-delivery measurements, not compositor scanout timing. Record the machine, browser, achieved color space and artifact hashes with results; a local pass does not promote camera/color qualification. Geometry-specific measurement remains in the lens/geometry lane until that feature is on main.
...
The execution gate selects 15 classes: `RenderSchedulingBackpressureTests`, `RenderRequestIdentityTests`, `RenderActivityTests`, `NativeDetailCancellationTests`, `DecodedCacheAutosaveTests`, `RenderedPreviewCacheWriteTests`, `RawRenderSourceTests`, `EditSessionNativeSizeSeedTests`, `EditSessionPreviewCeilingTests`, `CanvasMathTests`, `GpuPrescaledExtentTests`, `GpuReadbackFreshnessTests`, `GpuExitPreviewCacheTests`, `GpuPresentationOutcomeTests`, and `GpuSessionReplacementTests`. These use generated images, real temporary XMP/cache files, and the committed synthetic RAW; no private/100MP fixtures or UI authentication are required. Tests run serially with Metal enabled. The job fails on test failure, any skip, a missing class, or fewer than 67 executed cases; it does not impose a new hardware timing threshold or allow failures. Started/passed XCTest records and the final suite totals must agree. The uploaded xUnit report is derived from these validated serial XCTest log records (SwiftPM only emits Swift Testing XML in serial mode); the original log is uploaded alongside it.
