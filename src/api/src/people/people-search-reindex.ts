 ...
 * poll tick, and clear the dead-letter/attempt/processed-at bookkeeping so a
 * row that previously dead-lettered isn't permanently skipped and Settings →
 * Workers doesn't keep showing a stale last-processed timestamp for a stage
 * that's about to be retried clean. This is the same full five-field reset
 * (`version`/`attempts`/`last_error`/`processed_at`/`dead`) used by every
 * other stage re-arm site — `setPlaceOverride`/`setDescriptionOverride`
 * (`db/assets.repo.ts`), `reArmCacheStages` (`workers/dedupe.helpers.ts`),
 * and the migration re-arm helpers.
 *
