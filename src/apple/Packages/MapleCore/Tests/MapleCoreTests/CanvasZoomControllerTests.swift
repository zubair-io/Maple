...
final class CanvasZoomControllerTests: XCTestCase {
  ...
  func testViewportChangedIgnoresDegenerateSizes() {
    ...
    XCTAssertEqual(
      controller.session.previewSize, CGSize(width: 2000, height: 1600),
  ...
  func testViewportResizeRepushesVisibleRect() {
    ...
    XCTAssertEqual(
      controller.session.pixelScale, 1.0, accuracy: 1e-9,
  ...
  func testPinchFramesDoNotRetargetSessionUntilEnd() {
    ...
    XCTAssertEqual(
      controller.model.pixelScale, 1.0, accuracy: 1e-9,
      "live model scale moves per frame")
    XCTAssertEqual(
      controller.session.pixelScale, 0.25, accuracy: 1e-9,
  ...
  func testDragCommitsVisibleRectOnEndOnly() {
    ...
    XCTAssertEqual(
      controller.session.viewportSourceRect, rectBefore,
  ...
  func testWheelZoomCommitsImmediately() {
    ...
    XCTAssertEqual(
      controller.session.pixelScale, controller.effectivePixelScale,
      accuracy: 1e-9)
  }
  ...
  func testWheelPanEventsUpdateModelLiveWithoutCommitting() {
    ...
    XCTAssertNotEqual(
      controller.model.panOffset, .zero,
      "the live model must move on every event")
    XCTAssertEqual(
      controller.session.viewportSourceRect, rectAfterZoom,
  ...
  /// reflects the FINAL model state — not an intermediate one. Await the
  /// actual pending task instead of guessing when a fixed sleep resumes.
  func testWheelPanCommitsOnceAfterStreamGoesIdle() async {
    ...
    await controller._testPendingWheelPanCommitTask?.value

    XCTAssertEqual(
      controller._testWheelPanCommitFireCount, 1,
      "ten rapid events must coalesce into exactly one commit")
    XCTAssertEqual(
      controller.panOffset, panAtLastEvent,
    ...
    XCTAssertEqual(
      controller.session.viewportSourceRect.midX, 4200, accuracy: 1e-6,
      "the committed visible rect reflects the FINAL panned position")
  }

  /// Replacing a pending event must cancel its task and commit only the
  /// replacement. Both events are issued synchronously on MainActor, so
  /// scheduler load cannot let the first deadline fire between them.
  func testFreshWheelEventRestartsTheDebounce() async throws {
    let controller = makeController()
    controller.zoomToScale(1.0)
    controller._testSetWheelPanCommitInterval(milliseconds: 120_000)

    controller.wheelPan(delta: CGSize(width: -10, height: 0))
    let first = try XCTUnwrap(controller._testPendingWheelPanCommitTask)
    defer { first.cancel() }
    controller._testSetWheelPanCommitInterval(milliseconds: 0)
    controller.wheelPan(delta: CGSize(width: -10, height: 0))
    let replacement = try XCTUnwrap(controller._testPendingWheelPanCommitTask)

    XCTAssertTrue(first.isCancelled, "a fresh event must cancel the previous deadline")
    XCTAssertFalse(replacement.isCancelled, "the replacement must remain scheduled")
    XCTAssertEqual(
      controller._testWheelPanCommitFireCount, 0,
      "no commit fires while synchronous events are still arriving")

    await replacement.value
    XCTAssertEqual(
      controller._testWheelPanCommitFireCount, 1,
      "the replacement commits exactly once after its wait completes")
  }
  ...
  func testImmediateCommitCancelsPendingWheelPanCommit() async throws {
    let controller = makeController()
    controller.zoomToScale(1.0)
    controller._testSetWheelPanCommitInterval(milliseconds: 30)

    controller.wheelPan(delta: CGSize(width: -10, height: 0))
    let pending = try XCTUnwrap(controller._testPendingWheelPanCommitTask)
    ...
    XCTAssertEqual(
      controller.session.pixelScale, 1.25, accuracy: 1e-9,
  ...
  func testZoomCommandBeforeViewportDefersCommit() {
    ...
    XCTAssertEqual(
      controller.model.pixelScale, 1.25, accuracy: 1e-9,
      "the model holds the queued intent")

    controller.viewportChanged(points: CGSize(width: 1000, height: 800), displayScale: 2)
    XCTAssertEqual(
      session.pixelScale, 1.25, accuracy: 1e-9,
