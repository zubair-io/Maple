...
6. **Finish the work — no placeholder shortcuts.** Ship complete, working implementations. Never leave stubs, `TODO`/`FIXME` gaps, hard-coded fake data, empty handlers, or "wire this up later" holes dressed up to look done. If something genuinely can't be completed in one pass, **stop and say what's blocking** — don't paper over it. The only allowed exception is deliberate, incremental staging that is tracked by a referenced ticket and called out explicitly in the code (e.g. an overlay gated behind `#629` with a comment saying so); a silent placeholder is never acceptable. When in doubt, do the real thing. This applies at the epic level, not just the file level: when a piece of work was scoped into an ordered set of sub-issues (a plan, a design doc, an `AskUserQuestion` sequencing choice), don't stop and wait after landing the first slice and call it done — keep going through the remaining sub-issues in the same session unless something is genuinely blocking (a decision only the user can make, missing credentials/fixtures, a merge you're not authorized to make). "The next piece is a separate ticket" is not, on its own, a reason to stop; it's a reason to keep working and open the next PR. Sub-issues that don't depend on each other's output (e.g. independent Settings pages, independent Worker projects) should be built in parallel — via concurrent `Agent` dispatches or by just implementing them together in one pass — not serialized into artificial sprints when nothing about the design forces that order.
