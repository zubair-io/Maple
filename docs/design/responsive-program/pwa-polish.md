...
4. **Manifest hygiene** — `display: "standalone"`, `id: "/"`, `scope: "/"`, `start_url: "/browse"`, name / short_name, icons array (192, 512, plus a maskable 512), and a `protocol_handlers` entry for `web+maple` → `/library/editor/%s`. The `web+` prefix is required by the W3C Manifest spec (plain `maple` is silently dropped by Chromium); the iOS side keeps the bare `maple://` scheme per the deep-links spec — the web service parses both.
...
**`protocol_handlers`.** Per [W3C Manifest §protocol_handlers](https://www.w3.org/TR/manifest-app-info/#protocol_handlers-member), only safelisted protocols or `web+`-prefixed ones are accepted. We register `web+maple` → `/library/editor/%s` (image-id route). The iOS scheme stays bare `maple://` per the deep-links spec (`docs/design/responsive-program/deep-links.md`); when the web deep-link service lands it will need to accept both `maple://image/X` and `web+maple://image/X` and normalise. Chromium-only; Safari ignores the field.
