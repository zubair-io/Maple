...
public struct PipelineRenderer: Sendable {
    ...
    /// minutes. `full` uses bilinear demosaic (legacy export path, kept for
    /// parity harness compatibility). `amaze` uses the AMaZE demosaic for
    /// highest quality on Bayer images — this is the new export/refine path.
