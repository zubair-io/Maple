// RawPipelineService — legacy/non-RAW decode round-trip tests.
//
...
//
// The Plan 3 M1 `decodeSceneLinear()`/`decodeSceneLinearSized()` round-trip
// coverage that used to live here was deleted alongside those methods
// (#3048 — vestigial WebGL2 surface, no production caller remained).
//
...
describe('RawPipelineService — non-RAW browser-native decode (#784, #3039)', () => {
  ...
  // DECODER (rawler/demosaic). `decodeSceneLinear()`/`decodeSceneLinearSized()`
  // (the Plan 3 M1/M3 WebGL2 surface that used to browser-decode non-RAW
  // end to end) were deleted as dead (#3048 — no production caller
  // remained; `decodeNonRawToSceneLinear` itself stays in image-utils.ts
  // with its own LUT-exactness coverage below).
