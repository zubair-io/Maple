...
**Stage 25 carries two groups.** A local-adjustment layer's eleven POINT controls are per-pixel operators scaled by the mask weight, evaluated in one pass over the buffer. Its six SPATIAL controls (#3407 — texture, clarity, dehaze, sharpness, luminance noise, defringe) read a neighbourhood, so there is no weighted per-pixel form: `stages/local_adjustments/spatial.rs` runs the GLOBAL stage's own kernel over a scratch copy of that layer's output at the layer's slider value and lerps the result back by the mask weight (`out = base + w · (stage(base) − base)`). At `w = 1` that is bit-for-bit the global stage; at `w = 0` it is identity, and a layer that engages none of the six never allocates the scratch. `stages/defringe.rs` exists only for this group — Maple has no global Defringe slider. On the GPU the same structure is a per-layer split: when some layer engages the group, `raw-gpu`'s live chain emits one pass per layer instead of the fused single dispatch, and the engaged layer's pass drives the shipping clarity / texture / dehaze / sharpen / NLM / defringe kernels over scratch before blending by the weight `local_adjustments.wgsl` already writes into alpha for the vectorscope.
