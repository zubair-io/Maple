...
- **`web-webgpu-smoke`** (`web.yml`, `ubuntu-latest`) runs two existing dev-server e2e specs — `e2e/raw-open.spec.ts` (a real RAW decode with no WASM trap) and `e2e/slider-tick-render.perf.spec.ts` (opens a real live GPU/WASM render session and drives the Exposure slider through real per-tick renders) — in headless Chromium with `--enable-unsafe-webgpu` (set by the perf spec's own `launchOptions`, the flag that makes a WebGPU adapter resolve at all in headless Chrome). Both specs already skip-pass without the gitignored `test-fixtures/raws/test_0006.DNG`, the same convention as every other fixture-gated gate, so this job is a clean no-op on a stock runner today and becomes meaningful once fixtures are provisioned.
