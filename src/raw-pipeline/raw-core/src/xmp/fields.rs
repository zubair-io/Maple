//! Attribute → `AdjustmentModel` field mapping for the XMP walker.
//!
//! Split out of `xmp/mod.rs` in #365: the nested-element tone-curve path
//! pushed that file past the 600-line hard budget, and this `match` — one arm
//! per sidecar attribute, ~70 of them — is the part that grows on its own
//! schedule every time a slider ships. The contents are a pure move; the
