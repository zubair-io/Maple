...
Investigation for #3665, initially inspected at `9169b88bd` (2026-09-16); Apple persistence ownership below is updated through `306a8146bb` / #3729; Windows lifetime ownership is updated through `cd888ef05e` / #3730. This source map links the execution evidence separately; source inspection alone is not runtime qualification. At the original inspection, Windows `MainWindow*.cs` spanned 29 files / 5,932 physical lines; Apple `EditSession*.swift` spanned 28 files / 4,800 lines. Counts include comments and blank lines and exclude XAML, collaborators and tests. Partial declarations and Swift extensions share the same object; the file splits do not enforce ownership.
...
- The original Windows inspection found unpaired panel ownership and no awaited renderer shutdown. [#3730](https://github.com/zubair-io/Maple/pull/3730) repairs [#3702](https://github.com/zubair-io/Maple/issues/3702) with an ordered window-owned drain, late-producer guards and real WinUI lifecycle qualification.
...
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Construction, mode and disposal | [MainWindow.xaml.cs](../src/windows/Maple.WinUI/MainWindow.xaml.cs): constructor, `SetMode`, `EnterPreview`; `MainWindow.Lifetime` close/drain | `ViewModel`, `_mode`, `_settings`, `_panelNative`, renderer event subscriptions                                                                                            | XAML objects and mode transitions stay on UI thread; renderer must stop using panel before its reference is released.                 |
...
| Qualification                   | `Qualify`, `LifecycleSmoke`                                                                                                                    | `Qualify` starts from construction; `LifecycleSmoke` starts after app activation                                                                                           | `LifecycleSmoke` exercises normal close and native teardown; the older `Qualify` entry does not.                                      |
...
1. Constructor creates `EditSessionViewModel`, subscribes renderer/UI events and obtains `ISwapChainPanelNative` with `Marshal.QueryInterface`. The window owns that reference; the renderer borrows it.
2. [`MainWindow.Lifetime`](../src/windows/Maple.WinUI/MainWindow.Lifetime.cs) handles `Closed` synchronously, marks the window closing, calls view-model disposal to reject late producers, and defers final close while the dispatcher remains alive. Already-queued renderer callbacks check the closing/stopping guards.
3. [`EditSessionViewModel.Lifetime`](../src/windows/Maple.WinUI/ViewModels/EditSessionViewModel.Lifetime.cs) advances decode generation, requests native decode cancellation, stops the renderer and disposes timers/watchers. A decoder that raced the generation check still cannot reopen a stopped renderer.
4. [`RenderScheduler.Lifetime`](../src/windows/Maple.WinUI/Services/RenderScheduler.Lifetime.cs) retains the loop task. `StopAsync` marks stopping immediately, asynchronously joins the loop, then closes native sessions and clears the borrowed pointer. It never synchronously waits for a UI callback on the UI thread.
5. The window disconnects its CloudFiles connection off the UI thread without unregistering the persistent sync root. Its nested cleanup then awaits renderer teardown and releases the window's COM panel reference exactly once. Safe completed teardown allows normal final `Close`; errors remain logged and produce a failure exit code.

This ownership map does not claim a measured leak magnitude or hardware performance improvement. The boundary is a concrete resource-ordering guarantee: no new renderer work after stop, no native session surviving the join, and no panel release while the render loop still owns a pending presentation.

### Existing evidence and gaps

[`Maple.WinUI.Tests.csproj`](../src/windows/Maple.WinUI.Tests/Maple.WinUI.Tests.csproj) links portable logic/services into a .NET test project. Existing `ViewerFilmstripLogicTests`, `WhiteBalancePickLogicTests`, `SingleFlightGateTests`, `BatchRenameLogicTests`, `RenameReconciliationLogicTests`, `TrashSelectionLogicTests`, `RelocateCrashSafetyTests` and XMP tests cover useful extracted decisions and file contracts. They do **not** construct the WinUI `MainWindow` and therefore do not certify its event attachment, dispatcher callbacks or COM teardown. [#3730](https://github.com/zubair-io/Maple/pull/3730) adds a separate [actual-app smoke](../src/windows/Maple.WinUI/MainWindow.LifecycleSmoke.cs) and [Windows driver](../src/windows/scripts/test-window-lifecycle.ps1). It requires real HWNDs and native rendering, closes with an actual GPU present queued, checks repeated stop/close and late decoded results, and verifies normal process exit plus unchanged original RAW hashes. GPU, explicit CPU and empty-window scenarios each passed on the hosted Windows runner. The runner reported Microsoft Hyper-V Video: this is functional native/DX12 lifecycle evidence, not physical-device or performance qualification. The source revision, adapter, reports and logs are retained as CI artifacts.
...
This document changes no runtime behavior, pixel math, schema, actor boundary or scheduling policy. Both follow-ups preserve the existing platform ownership boundaries and carry separate execution evidence. Neither is a mandate to split every large type.
