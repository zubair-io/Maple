...
public struct MuiDragBar: View {
    ...
    VStack(alignment: .leading, spacing: 4) {
      ...
      GeometryReader { geo in
        ...
        ZStack(alignment: .leading) {
          ...
          let markerPct =
            MuiScrubMath.percentInRange(
              value: value, min: range.lowerBound, max: range.upperBound, fallbackPct: 50) / 100
          Rectangle()
        ...
        .gesture(
          DragGesture(minimumDistance: 0)
            .onChanged { g in
              ...
              value = Self.valueAtPosition(
                x: g.location.x, barWidth: barWidth, range: range, step: step)
            }
        )
      }
      .frame(height: 30)
      ...
      .accessibilityAdjustableAction { direction in
        ...
        switch direction {
        case .increment:
          value = MuiScrubMath.snap(
            value + step, step: step, min: range.lowerBound, max: range.upperBound)
        case .decrement:
          value = MuiScrubMath.snap(
            value - step, step: step, min: range.lowerBound, max: range.upperBound)
  ...
  public static func valueAtPosition(
    x: CGFloat, barWidth: CGFloat, range: ClosedRange<Double>, step: Double
  ) -> Double {
