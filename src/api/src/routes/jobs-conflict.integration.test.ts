...
function jobCount(): number {
  return (
    database.db.query(`SELECT COUNT(*) AS n FROM jobs`).get() as { n: number }
  ).n;
}

function batchPayload(name = "photo") {
  return {
    targets: [{ id: `conflicts:${name}.jpg`, path: join(root, `${name}.jpg`) }],
    patch,
  };
}
...
async function failedBatch() {
  ...
  const previous = await createJob(
    { kind: "batch_adjustment_sync", payload },
    undefined,
    db,
  );
  await markCancelled(previous._id, null, undefined, undefined, db);
  const frozenPatch = {
    attributes: { "crs:Exposure2012": "2.5" },
    elements: {},
  };
  ...
  run(
    ...
    JSON.stringify({
      ...
      entries: [
        { id: payload.targets[0].id, status: "failed", patch: frozenPatch },
      ],
    }),
...
describe("job creation conflicts", () => {
  isolatedIt(
    "rejects a malformed batch target before path authorization",
    async () => {
      ...
      expect((await response.json()).error).toContain(
        "Every photo needs an id and an absolute path",
      );
    },
  );

  isolatedIt(
    "rejects targets whose aliased paths resolve to one sidecar",
    async () => {
  ...
  isolatedIt(
    ...
    async () => {
      ...
      expect((await getJob(new ObjectId(requestId), db))?.payload).toEqual(
        original.payload,
      );
      expect((await post("", original)).status).toBe(201);
    },
  ...
  isolatedIt(
    "fences active batches submitted through aliased library registrations",
    async () => {
      ...
      try {
        ...
        const response = await post("", {
          ...
          payload: {
            targets: [
              {
                id: "conflicts-alias:aliased.jpg",
                path: join(alias, "aliased.jpg"),
              },
            ],
            patch,
          },
  ...
  isolatedIt(
    "returns 409 when a retry request id belongs to another job",
    async () => {
      const { previous } = await failedBatch();
      const requestId = new ObjectId().toHexString();
      await createJob(
        { kind: "batch_jpeg_export", payload: {}, requestId },
        undefined,
        db,
      );
      ...
      expect(response.status).toBe(409);
      expect((await response.json()).error).toContain("different job");
      expect((await getJob(new ObjectId(requestId), db))?.kind).toBe(
        "batch_jpeg_export",
      );
    },
  );
  ...
  isolatedIt(
    "returns 409 when a retry overlaps an active library batch",
    async () => {
      const { previous } = await failedBatch();
      await createJob(
  ...
  isolatedIt(
    ...
    async () => {
      ...
      expect(created?.payload.targets).toEqual([
        { ...payload.targets[0], patch: frozenPatch },
      ]);
  ...
  isolatedIt(
    "preserves unrelated database failures as 500 on both creation routes",
    async () => {
      const { previous, payload } = await failedBatch();
      database.db.run(
        `CREATE UNIQUE INDEX test_unrelated_conflict ON jobs (kind)`,
      );
      try {
        expect(
          (await post("", { kind: "batch_adjustment_sync", payload })).status,
        ).toBe(500);
        expect((await post(`/${previous._id}/retry-failed`, {})).status).toBe(
          500,
        );
      } finally {
