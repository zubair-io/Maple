...
A separate inpaint-removal model exists in `src/raw-pipeline/raw-core/src/types/inpaint.rs` and **no platform exposes it** — the web tool dock renders Heal as an explicitly disabled, `aria-hidden` entry (`components/editor/tool-dock.component.ts`), and Apple's `ToolDock.swift` does the same.
