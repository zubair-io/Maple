...
describe('trash-gc', () => {
  ...
  test('purges files + docs older than the retention window; preserves fresh', async () => {
    ...
    const fi = (p: string, fn: string) => [
      { library_id: libraryId, path: p, filename: fn, deleted_at: null },
    ];
    await db!.collection('assets').insertMany([
      {
        _id: oldId,
        fileinfo: fi('.maple/trash', 'old.ARW'),
        size: 1,
        mtime: 0,
        indexed_at: days(60),
        deleted_at: days(31),
        original_path: '/x/old.ARW',
      } as never,
      {
        _id: freshId,
        fileinfo: fi('.maple/trash', 'fresh.ARW'),
        size: 1,
        mtime: 0,
        indexed_at: days(60),
        deleted_at: days(7),
        original_path: '/x/fresh.ARW',
      } as never,
      {
        _id: liveId,
        fileinfo: fi('', 'live.ARW'),
        size: 1,
        mtime: 0,
        indexed_at: days(1),
        deleted_at: null,
