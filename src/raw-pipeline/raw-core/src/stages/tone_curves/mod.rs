...
use evaluator::{eval_curve_scene_linear, prepare_curve};
...
fn apply_parametric_luma_coupled(img: &mut Image, model: &AdjustmentModel) {
    ...
    for p in &mut img.pixels {
        ...
        let y_new = eval_curve_scene_linear(&knots, y_old);
...
fn apply_point_curve_luma_coupled(img: &mut Image, curve: &ToneCurve) {
    // Lift the points into a sortable Vec — the evaluator requires
    // x-monotonic input. Cheap (one allocation per stage call; the per-
    // pixel inner loop reads the prepared slice).
