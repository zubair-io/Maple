...
Investigation for #3665, inspected at `9169b88bd` (2026-09-16). This is a source map, not runtime qualification. Windows `MainWindow*.cs` still spans 29 files / 5,932 physical lines; Apple `EditSession*.swift` spans 28 files / 4,800 lines. Counts include comments and blank lines and exclude XAML, collaborators and tests. Partial declarations and Swift extensions share the same object; the file splits do not enforce ownership.
...
- Apple preview persistence has a small, demonstrable ownership seam: three shared properties are coordinated by CPU and GPU persistence extensions. Encapsulate that state while retaining session-level exit ordering in [#3703](https://github.com/zubair-io/Maple/issues/3703).
...
flowchart LR
    ...
    Publish --> Session
    Session --> Persist[Pending preview / debounce / sink]
...
For #3703, retain these contracts and explicitly exercise an already-running encode overlapping exit, a model change across an await and final-write survival through session teardown. Keep model/generation checks in the session; the proposed owner only encloses pending preview state and its existing task.
...
No runtime behavior, pixel math, schema, actor boundary or scheduling policy changes in this investigation. The two linked follow-ups are intentionally bounded; neither is a mandate to split every large type.
