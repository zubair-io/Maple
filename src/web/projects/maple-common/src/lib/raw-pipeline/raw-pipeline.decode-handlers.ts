...
/**
 * Choose and run the render for a legacy display-encoded decode: GPU one-shot
 * (with its CPU-sized adapter-failure retry), viewport-sized CPU, or unsized
 * CPU — bracketed by the worker-local perf mark pair. Split from
 * `handleLegacyDecode` so the route selection and the response envelope stay
 * independently readable.
 */
interface LegacyRoute {
  readonly sized: boolean;
  readonly gpuFn: RenderBytesGpuFn | null;
  readonly markTag: string;
}

/** The pure route decision for a legacy decode — see `renderLegacy`. */
function legacyRoute(req: DecodeRequest): LegacyRoute {
  ...
  const markTag = gpuFn
    ...
    : sized
      ...
      : 'maple:wasm';
  return { sized, gpuFn, markTag };
}

async function renderLegacy(req: DecodeRequest, bytes: Uint8Array): Promise<MapleRender> {
  const { sized, gpuFn, markTag } = legacyRoute(req);
  // #1123: markStart/markEnd — a Performance Timeline throw here (e.g. a cleared
  // mark) must never fall through to the caller's `catch` and mislabel a
  // successful decode as a `decode-error`.
  const wasmStartMark = `maple:wasm:${req.id}:start`;
  markStart(wasmStartMark);
  let result;
  if (gpuFn) {
    result = await renderLegacyGpu(gpuFn, req, bytes);
  } else if (sized) {
...
/** The GPU one-shot render with its CPU-sized adapter-failure retry. */
async function renderLegacyGpu(
  ...
  try {
    ...
    // instance outright on ≥~35 MP sensors). The caller's catch still handles
    // a genuine CPU decode failure.
    console.warn(
      '[raw-pipeline.worker] GPU render failed; falling back to CPU render_bytes_sized:',
      gpuErr,
    );
    return render_bytes_sized(bytes, req.ext, req.xmp ?? null, false, default_target_long_edge());
  }
}

/**
 * The response scalars both render result types carry — read BEFORE the
 * buffer take/free (the scalar getters stay valid after, but reading first
 * keeps the take-then-free sequence unconditional).
 */
function readRenderScalars(result: MapleRender | MapleSceneLinearRender) {
  return {
    width: result.width,
...
export async function handleLegacyDecode(
  ...
  try {
    await ensureReady();
    const result = await renderLegacy(req, new Uint8Array(req.bytes));
    // Read the scalars, then consume the RGB via `take_rgb()` (#1080): unlike
    // the `rgb` getter (which CLONES the full frame and leaves the wasm copy
    // alive until free/GC), the take MOVES the bytes out, so peak memory is
    // one frame, not two. The explicit `free()` releases the wasm-side struct
    // immediately instead of waiting on GC finalization.
    const scalars = readRenderScalars(result);
...
export async function handleSceneLinearDecode(
  ...
  try {
    await ensureReady();
    const result = renderSceneLinear(req, new Uint8Array(req.bytes));
    // Read the scalars, then consume the lanes via `take_fp16_rgba()` (#1080):
    // unlike the `fp16_rgba` getter (which CLONES the full frame and leaves
    // the wasm copy alive until free/GC), the take MOVES them out, so peak
    // memory is one frame, not two. wasm-bindgen returns a Uint16Array; slice
    // its underlying buffer so we can transfer it (avoid the main thread
    // holding a copy).
    const scalars = readRenderScalars(result);
...
/**
 * The sized/unsized scene-linear render, bracketed by the worker-local perf
 * mark pair (mirrors the legacy `maple:wasm` entry; #1123: markStart/markEnd —
 * see `renderLegacy`). Sized routing (#1101, spec §5.1):
 * `render_bytes_scene_linear_sized` is the WASM mirror of the Apple FFI's
 * `maple_render_bytes_scene_linear_sized` (same raw-core path — downsample
 * lands right after demosaic).
 */
function renderSceneLinear(
  req: DecodeSceneLinearRequest,
  bytes: Uint8Array,
