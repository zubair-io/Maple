...
After building the Hosted artifact, run `cd src/web && bun run check:cold-export-lens`. This browser regression imports authored calibrations into real IndexedDB, destroys the importing worker, and sends an immutable export XMP snapshot as the replacement worker's first request. It compares actual PNG pixels with the original calibration and a later selection, checks missing/corrupt-cache errors and disabled/all-zero identity, and verifies the committed synthetic RAW remains unchanged. It requires Chrome and a fresh WASM/Hosted build, and never silently skips.
