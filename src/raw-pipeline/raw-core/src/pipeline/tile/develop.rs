...
pub(super) fn develop_scene_linear_from_padded_mosaic(
    ...
    let (profile, tier) = stage("tile_dcp_profile_for", || dcp::profile_for_with_tier(raw))?;
    // No source-mixing — see `pipeline::develop` for rationale. At
    // BundleConfident the file's PTC/PLT don't match the bundle's matrices,
    // so both are dropped; at the embedded tiers they're coherent and kept.
