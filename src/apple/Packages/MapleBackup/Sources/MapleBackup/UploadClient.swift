    ...
    while offset < total {
      ...
      case 409:
        ...
        struct Mismatch: Decodable {
          ...
          enum CodingKeys: String, CodingKey { case expectedOffset = "expected_offset" }
        }
        if let body = try? JSONDecoder().decode(Mismatch.self, from: data),
          let newOffset = body.expectedOffset
        {
          if newOffset >= total {
      ...
      case 423:
        ...
        struct Busy: Decodable {
          ...
          enum CodingKeys: String, CodingKey { case retryAfterSeconds = "retry_after_seconds" }
        }
      ...
      case 200:
        ...
        struct Final: Decodable {
          ...
          enum CodingKeys: String, CodingKey {
            ...
            case targetRelPath = "target_rel_path"
          }
        }
        let body = try JSONDecoder().decode(Final.self, from: data)
        return Result(mapleId: body.mapleId, targetRelPath: body.targetRelPath)
      default:
        let snippet = Self.bodySnippet(data)
        Self.logger.error(
