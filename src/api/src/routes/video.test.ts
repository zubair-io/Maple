...
import { isStreamedFilePath } from "./library/shared.ts";
...
describe("GET /api/video/fs", () => {
  ...
  beforeEach(async () => {
    ...
    token = await signAccessToken(
      { sub: "u1", email: "user@example.com", role: "member" },
      SECRET,
    );
  });
  ...
  // #2382 regression. The suite above mounts `videoRoutes` bare, which is why
  // it stayed green while every Apple device failed to play video: the bug
  // only appears once the real middleware stack is present. Elysia rebuilds a
  // returned `Response` whenever `set.headers` is non-empty, discarding the
  // `BunFile` slice — so the 206 shipped the WHOLE file while still
  // advertising a partial `Content-Range`, which AVFoundation rejects.
  //
  // Mount `requestContext` (it writes `X-Request-Id`) AND the app's CORS hook,
  // not a stand-in: an earlier version of this test used a simulated one-header
  // hook, passed, and missed that `requestContext` was independently keeping
  // `set.headers` non-empty. Assert the exact byte count.
  test("serves exactly the requested bytes through the real middleware", async () => {
    const response = await new Elysia()
      ...
      .handle(
        new Request(
          `http://localhost/api/video/fs?path=${encodeURIComponent(file)}&token=${token}`,
          {
    ...
    expect(new TextDecoder().decode(body)).toBe("2345");
  });
  ...
  test("serves a byte range from the slug route", async () => {
    const response = await new Elysia().use(videoRoutes).handle(
      new Request(
        `http://localhost/api/video/video-test/clip.mp4?token=${token}`,
        {
