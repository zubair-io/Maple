...
- **SwiftUI shell.** AppShell (three-column split on Mac/iPad, bottom-tab collapse on iPhone), Browse grid, FullImage view, DetailPanel with Info + Develop tabs. UI design is defined by `src/web/` prototype (port the look-and-feel, not the React structure).
