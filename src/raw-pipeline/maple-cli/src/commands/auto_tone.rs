...
//! (`render_scene_linear_from_raw_with_quality_f32` with the default
//! `AdjustmentModel`), and hands the resulting RGBA f32 buffer to
...
pub fn run(raw_path: &Path) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    // a deterministic post-orientation RGBA f32 buffer.
