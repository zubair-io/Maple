...
- [ ] **Step 1: Reuse the shared harness `src/api/src/workers/derivative-audit/test-support.ts`** (`setupAuditMongo`), created in Task 2. There is NO `test-helpers/mongo.ts` and NO `setDbForTests` seam — that was a wrong assumption. The codebase's real pattern (see `cloudflare/cloudflare-config.repo.test.ts` and `people/people-repo.test-helpers.ts`) is: connect to `MAPLE_MONGO_URI` (default `mongodb://localhost:27017`), point the code-under-test's `getDb()` at a per-suite DB by setting `process.env.MAPLE_MONGO_DB` + calling `closeDb()`, and **skip-pass** (`if (!h.mongoReachable) return;`) when Mongo is unreachable. `setupAuditMongo` registers the `beforeAll`/`beforeEach`/`afterAll` hooks and exposes `{ mongoReachable, db, addLibrary(root, slug?) }` (the last inserts a `folders` doc + `invalidateLibraryRoots()` so `loadLibraryRoots`/`loadLibraryIdToSlug` resolve it). A local `mongod` is already running on `:27017`.
