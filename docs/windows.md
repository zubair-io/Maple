...
`MainWindow` follows a three-stage flow declared by `ShellMode` (`ShellMode.cs`) and driven from `MainWindow.xaml.cs`: **Browse** (grid) → **Preview** (full image, filmstrip rail, culling) → **Edit** (full-bleed canvas, the same filmstrip rail, tool rail, group panels). Sliders exist only in Edit. Preview and Edit share one Maple.UI `MuiFilmstripRail` on the canvas's left edge (`MainWindow.Filmstrip.cs`, #3402): it lists the current `Photos` in grid order, its active cell follows `SelectedPhoto`, and clicking a cell only changes the selection — Preview stays Preview, Edit stays Edit and re-decodes the new photo. The WinUI-free `ViewerFilmstripLogic` holds those two rules so Maple.WinUI.Tests can pin them. `MainWindow.xaml` (~900 lines) holds the layout: a custom menu strip that doubles as the title bar, a sidebar with a Timeline entry and two folder trees (Maple Cloud, Folders), the photo `GridView`, and the viewer with both an `Image` and a `SwapChainPanel` stacked so the CPU and GPU paths can swap. `Maple.UI.MuiWindowChrome.Extend` claims the caption strip and pins the caption-button colors to the Maple tokens.
...
Imported lens profiles (#2435 / #3480) are decode-owned too. `Native/RawFfi.LensProfile.cs` mirrors the five `maple_lens_profile_*` entries (register / resolve_file / selected / clear_cache, plus the JSON free), and `Services/LensProfileStore.cs` keeps the imported bytes under `%LOCALAPPDATA%\Maple\LensProfiles\<BLAKE3>.lcp`, addressed by the digest the register call returns. `RenderEngine.Decode` calls `LensProfileStore.RestoreForFile` before the native decode — warm process cache first, otherwise re-register from the store — and a profile the sidecar names but the device does not hold is a `LensProfileException`, surfaced verbatim as the decode failure and in the Lens panel; Maple never substitutes a profile or renders without the correction the sidecar asked for. After the decode, `DecodedImage.LensProfile` carries what the core resolved (source, per-family coverage, approximations, unsupported records), which `EditSessionViewModel.LensProfile.cs` turns into the panel text and the per-row enablement of the Distortion / Chromatic Aberration / Vignetting sliders (`AdjustmentSliderViewModel.IsEnabled`). The UI is `MainWindow.LensProfile.cs`: the Lens rail group's toggle, import picker, inspection dialog (the acknowledged `lcp1-ack:` spelling needs its own acceptance box and is only offered for an approximate match) and the current selection. `Services/Export/NativeExportRecipeExecutor.cs` restores the profile from the queued item's frozen XMP through `maple_lens_profile_selected` — the render-side parser — before `maple_export_recipe_to_file`.

That decode always requests Preview quality (a half-res 2×2-binned demosaic) first, whether it is the cold open or a re-decode triggered by a decode-owned field change (`DecodeInputsChanged` — profile, AE, lens corrections) — the first paint must land inside CLAUDE.md's 250–1000ms uncached-open budget, never wait on a full demosaic. That Preview base feeds both display phases immediately: the refine (settled) pass directly, and the fast slider-tick pass via a further software 2× box-average downsample (`RenderEngine.DownsampleHalf`).

Once the Preview decode is on screen, `EditSessionViewModel.RefineUpgrade.cs`'s `ScheduleAmazeUpgrade` (#3417) checks `RefineDecodeQuality.ShouldScheduleAmazeUpgrade` — mirroring Apple's `ImageEditPipeline.refineDecodeQuality` — against the sensor's native long edge (read from the file's EXIF/TIFF IFDs via `RenderEngine.SensorLongEdge`/`Services/Metadata/ExifReader.cs`, no decode needed). Preview's own output cap is roughly half that native long edge regardless of the requested target, so whenever the session's decode target exceeds it, a SECOND, cancellable decode runs in the background at AMaZE quality (the export path's quality), reusing the first decode's already-fitted Auto Profile tail rather than re-fitting. When it lands, `Renderer.SetImage` swaps it in as the new base for both phases — re-uploading/reopening the GPU sessions (or, on the CPU fallback, simply becoming the new refine/fast source) — so the refine pass and a 100% zoom both then read AMaZE detail. The upgrade is generation-guarded: a photo switch or another decode-owned edit signals its cancel flag and the stale result is dropped on arrival rather than swapped in over a newer base. Below the half-native-long-edge threshold, Preview already delivers exactly the requested target (no cap ever engages) and no upgrade is scheduled at all.
...
Cloud browsing (`EditSessionViewModel.CloudBrowse.cs`) walks the same routes the Apple cloud source walks — `GET /api/folders` for the roots, `GET /api/fs/dir` for each level — deliberately not `/api/search`, which returns a capture-sorted feed with no directory structure. Thumbnails and previews are fetched by `slug:relPath` address (`GET /api/thumb/:slug/*`, `GET /api/preview/:slug/*`), the unified routes the web grid reads, so the two clients share one server cache entry per image (#1325). The listing and the original bytes stay on the path-addressed routes on purpose: `/api/folder/:slug/*` reports none of the size, mtime or EXIF the grid renders and no absolute path (which still keys `/api/xmp?path=`, `/api/preview?path=` and `/api/assets/by-fspath`), and only `/api/fs/raw` carries the #926 mirror read-failover, so download-to-edit keeps working from a mirror when the primary volume is unmounted. Both become a route swap once the server closes those gaps. See [server-api](server-api.md).
...
- **`FETCH_PLACEHOLDERS`** — on-demand directory population from `GET /api/fs/dir`, the only listing that reports the size and mtime a placeholder is stamped with (a wrong `FileSize` breaks hydration). Each placeholder's `FileIdentity` is the entry's absolute server path, so a callback can address the server with no local state.
- **`FETCH_DATA`** — on-demand hydration. Opening a placeholder streams `GET /api/fs/raw` straight into `CfExecute` `TRANSFER_DATA` chunks; nothing spools to disk first. Explorer's "Always keep on this device" rides the same callback under the FULL hydration policy. Neither `/api/fs/raw` nor `/api/image` honours a `Range` header, so hydration serves whole-file requests from offset 0 only and refuses anything else; the offset rule and the 4096-aligned chunking loop live in `CloudFilesHydration.cs`, which `Maple.WinUI.Tests` links and pins (`CloudFilesHydrationTests`). The route stays `/api/fs/raw` rather than `/api/image/:slug/*` because only the former carries the #926 mirror failover.
...
The imported-lens-profile tests split the same way: `XmpLensProfileTests` and `LensProfileStoreTests` need no native core (reference shape, the needs-the-bytes rule, the resolve-JSON mapping, decode-owned invalidation), while `LensProfileStoreNativeTests` and `ExportLensProfileNativeTests` are armed by `MAPLE_RAW_FFI_DLL` like `RawFfiLayoutTests` and otherwise skip-pass. `ExportLensProfileFixture` authors the RAW/LCP pair from the committed synthetic grey DNG (capture identity appended as real IFD entries) plus a GUID-unique profile, so nothing user-owned is touched and the cold-process export test — a fresh `dotnet vstest` child with an empty native cache — can prove the queued snapshot restores from the store.
