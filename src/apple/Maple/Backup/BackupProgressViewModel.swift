...
public final class BackupProgressViewModel {
  ...
  /// — Stopped / Starting / Running. It is driven by
  ...
  public enum BackupRunPhase: Sendable {
    ...
    case running
    /// Startup is in flight (teardown + durable queue recovery).
    case starting
  ...
  /// Current engine phase. Driven by `EngineHost.start`/`stop`.
  ...
  public struct InFlight: Identifiable, Hashable, Sendable {
    ...
    public var preparation = "Reading Photos…"
    public var bytesTotal: Int64

    public init(
      id: BackupTaskID, startedAt: Date = Date(),
  ...
  public func removePendingPhotoIDs(_ ids: Set<String>) { pendingPhotoIDs.subtract(ids) }
  ...
  public func setPhase(_ newPhase: BackupRunPhase) {
    phase = newPhase
    // Throughput is a live "right now" readout — a stopped or starting
    ...
    if newPhase == .stopped || newPhase == .starting {
      resetThroughput()
      // Same rationale as `markPaused`: a stopped/starting engine has no
  ...
  public var walkPhaseLabel: String? {
    ...
    case .checkingServer:
      return "Checking existing backups on the server…"
    case .reconciling(let checked, let total):
      return "Checking \(checked.formatted()) of \(total.formatted()) photos for backup…"
  ...
  public var progressLabel: String {
    ...
    if totalEnqueued == 0 {
      if let failures = lastWalkSummary?.failedPermanently, failures > 0 {
        return "\(failures.formatted()) photos need attention"
      }
  ...
  func apply(_ event: BackupQueueEvent) {
    ...
    case .preparing(let id, let message):
      if let index = inFlight.firstIndex(where: { $0.id == id }) {
        inFlight[index].preparation = message
      }
    case .progress(let id, let sent, let total):
      ...
      meter.recordProgress(
        task: id, cumulativeSent: sent,
    ...
    case .cancelled(let id):
      pendingPhotoIDs.remove(id.phassetLocalId)
      inFlight.removeAll { $0.id == id }
      if seenEnqueued.remove(id) != nil { totalEnqueued -= 1 }
