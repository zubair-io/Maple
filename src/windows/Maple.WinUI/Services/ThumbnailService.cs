...
namespace Maple.WinUI.Services
{
    ...
    public sealed class ThumbnailService
    {
        ...
        public string CachePathFor(string rawPath)
        {
            var info = new FileInfo(rawPath);
            var key = $"{rawPath.ToLowerInvariant()}|{info.LastWriteTimeUtc.Ticks}|{info.Length}|{ThumbnailMaxPx}";
            var hash = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(key)))[..32];
            return Path.Combine(_cacheDir, $"{hash}.jpg");
        }

        /// <summary>Returns the cached thumbnail path, extracting it on first
        /// use. Null when the source has no embedded preview or extraction fails.</summary>
        public async Task<string?> GetOrCreateAsync(string rawPath, CancellationToken ct)
        {
            var cachePath = CachePathFor(rawPath);
            ...
            try
            {
                ...
                return await Task.Run(() =>
                {
                    var rc = RawFfi.maple_render_thumbnail_preview_jpeg_to_file(
                        rawPath, cachePath, ThumbnailMaxPx, 0);
                    if (rc != 0)
                    {
