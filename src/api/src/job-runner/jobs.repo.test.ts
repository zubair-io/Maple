...
const TEST_DB = withTestDb(`maple_test_jobs_repo_${process.pid}`);
...
describe('jobs.repo', () => {
  ...
  it('listJobs filters by status and kind', async () => {
    ...
    expect(queued.map((j) => j._id.toHexString())).toEqual([b._id.toHexString()]);
    const done = await repo.listJobs({ status: 'done' });
    expect(done.map((j) => j._id.toHexString())).toEqual([a._id.toHexString()]);
  });
});
