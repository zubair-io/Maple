...
- **Auto Adjustments** (`stages/auto_adjustments.rs`) is a one-shot recommendation for all eight tone/WB sliders, derived from one probe develop with auto-exposure off and WB pinned to D65. Its `exposure` output _replaces_ the anchor, so a caller writing it back must also set `papp:AutoExposure="Off"`.
