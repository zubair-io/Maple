...
describe('POST /api/libraries/:id/backup/ingest — happy paths', () => {
  ...
  test('backup upload with spec-form maple_id matching pre-seeded AssetDoc → dedup, no second file', async () => {
    ...
    // The backup-ingest route shorts on `{ maple_id }`, but it ALSO
    // resolves `assetAbsPath` from fileinfo to confirm the on-disk
    // file matches — so the seed must point at the same indexerAbsPath.
    await a.insertOne({
      _id: new ObjectId(),
      fileinfo: [
        {
