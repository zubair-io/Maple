...
// Tests live in the sibling `image_tests.rs` so this file stays under the
// 600-LOC budget (#2231's has_lens_corrections/lens_correction_ca_inert
// tests pushed it over). Same `#[path]` split pattern as
// `stages/auto_adjustments.rs` (`auto_adjustments_tests.rs`, PR #1730).
#[cfg(test)]
#[path = "image_tests.rs"]
mod tests;
