...
fn negative_baseline_exposure_recovers_sensor_clipped_green() {
    ...
    img.pixels.fill([0.9 * gain; 3]);
    let center = 5 * 11 + 5;
    img.pixels[center] = [1.6 * gain, gain, gain];
    let original = img.pixels[center];
    ...
    let recovered = img.pixels[center];
...
#[test]
fn positive_baseline_exposure_does_not_reconstruct_unclipped_pixels() {
    let gain = 0.25_f32.exp2();
    let mut img = Image::new(11, 11, ColorSpace::CameraNativeLinearRgb);
    img.pixels.fill([0.3 * gain; 3]);
    // Green exceeds the old 1.0 ceiling but is only 90% of sensor white.
    img.pixels[5 * 11 + 5] = [0.8 * gain, 0.9 * gain, 0.7 * gain];
...
#[test]
fn fully_clipped_white_tracks_baseline_exposure_with_and_without_pre_gain() {
    for neutral in [NEUTRAL_DAYLIGHT, NEUTRAL_IDENTITY] {
        for baseline in [-0.78_f32, 0.0, 0.25] {
            let gain = baseline.exp2();
            let sensor_white = neutral.map(|n| gain / n);
            let expected = sensor_white.into_iter().fold(0.0_f32, f32::max);
            let mut img = Image::new(1, 1, ColorSpace::CameraNativeLinearRgb);
            img.pixels[0] = sensor_white;
...
#[test]
fn negligible_baseline_matches_the_develop_gain_fast_path() {
    let unscaled = ceilings(NEUTRAL_DAYLIGHT, 0.0);
    for baseline in [-1e-4, 1e-4] {
        assert_eq!(ceilings(NEUTRAL_DAYLIGHT, baseline), unscaled);
    }
}

#[test]
fn off_preserves_pixels_at_nonzero_baseline_exposure() {
    let mut img = Image::new(1, 1, ColorSpace::CameraNativeLinearRgb);
    img.pixels[0] = [0.9, 0.58, 0.58];
    let before = img.pixels.clone();
    apply(
        &mut img,
...
fn mode_off_is_identity() {
    ...
    let before = img.pixels.clone();
    apply(&mut img, HighlightRecoveryMode::Off, NEUTRAL_DAYLIGHT, 0.0);
    assert_eq!(img.pixels, before);
}
