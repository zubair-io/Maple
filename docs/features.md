...
**AUTO** analyses the RAW and writes **exposure plus the five calibrated tone sliders** (contrast, highlights, shadows, whites, blacks), as a single undo entry with `autoExposure: 'Off'`. Apple and web preserve the current white balance. The AUTO button lives in the shared Apple editor header on Mac, iPad and iPhone; it shows progress and disables repeat taps during analysis. Web exposes it in the editor top bar. Windows exposes Edit → Auto Adjust and also applies the analysis's white-balance estimate.
