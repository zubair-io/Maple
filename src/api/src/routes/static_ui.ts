...
function resolveUiDist(): string {
  if (process.env.MAPLE_UI_DIST) {
    // Normalize via path.resolve so the containment check in serveStatic is
    // robust: it strips any trailing slash and makes the value absolute, to
    // match path.join's output. Without this, MAPLE_UI_DIST="/app/dist/"
    // would 403 every request (UI_DIST + path.sep would be "/app/dist//",
    // which the normalized filePath never starts with).
