...
describe('GET /api/video/fs', () => {
  ...
  // it stayed green while every Apple device failed to play video: the bug only
  // appears once the real middleware is present. Elysia rebuilds a returned
  // `Response` whenever `set.headers` is non-empty, discarding the `BunFile`
  // slice — so the 206 shipped the WHOLE file while still advertising a partial
  // `Content-Range`, which AVFoundation rejects.
  //
  // Mount the REAL `requestContext` (it writes `X-Request-Id`) plus the CORS
  // hook, not a stand-in: an earlier draft used a simulated one-header hook,
  // passed, and missed that `requestContext` was independently keeping
