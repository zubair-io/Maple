...
//
// AUTO is mounted in the shared PillHeader again (#3249). The phone header
// scrolls its trailing controls, keeping this action reachable at compact width.

import XCTest

final class EditorAutoResetReachabilityTests: XCTestCase {

  func testSharedEditorHeaderMountsAccessibleAutoControl() throws {
    let sources = try viewSources()
    let header = try XCTUnwrap(sources.first { $0.name == "PillHeader.swift" })
    let auto = try XCTUnwrap(sources.first { $0.name == "EditorAutoButton.swift" })
    XCTAssertTrue(header.text.contains("EditorAutoButton(state: state)"))
    XCTAssertTrue(auto.text.contains("await state.applyAuto()"))
