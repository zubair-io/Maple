...
import { requestContext, errorEnvelope } from '../src/middleware/request-context.ts';
...
describe('requestContext: concurrency safety', () => {
  test('100 concurrent requests with distinct supplied ids each echo their own id (no cross-talk)', async () => {
    ...
    const app = appWith((a) =>
      a.get('/concurrent', async ({ set }: { set: { status: number } }) => {
...
describe('errorEnvelope', () => {
  test('correctly maps common status codes to error codes', () => {
    const rid = 'test-request-id';
    const msg = 'test-message';

    expect(errorEnvelope(400, msg, rid).code).toBe('bad_request');
    expect(errorEnvelope(401, msg, rid).code).toBe('unauthorized');
    expect(errorEnvelope(403, msg, rid).code).toBe('forbidden');
    expect(errorEnvelope(404, msg, rid).code).toBe('not_found');
    expect(errorEnvelope(409, msg, rid).code).toBe('conflict');
    expect(errorEnvelope(415, msg, rid).code).toBe('unsupported_media_type');
    expect(errorEnvelope(422, msg, rid).code).toBe('validation');
    expect(errorEnvelope(503, msg, rid).code).toBe('service_unavailable');
    expect(errorEnvelope(500, msg, rid).code).toBe('internal');
  });

  test('defaults unknown 4xx to bad_request and unknown 5xx to internal', () => {
    const rid = 'test-request-id';
    const msg = 'test-message';

    expect(errorEnvelope(418, msg, rid).code).toBe('bad_request');
    expect(errorEnvelope(501, msg, rid).code).toBe('internal');
  });

  test('sets message and requestId correctly', () => {
    const rid = 'test-request-id';
    const msg = 'test-message';
    const env = errorEnvelope(400, msg, rid);

    expect(env.error).toBe(msg);
    expect(env.requestId).toBe(rid);
  });
  ...
  test('includes details when provided', () => {
    ...
    const env = errorEnvelope(400, msg, rid, details);
  ...
  test('omits details when not provided', () => {
    ...
    expect(Object.keys(env)).not.toContain('details');
  });
});
