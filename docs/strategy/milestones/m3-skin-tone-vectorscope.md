...
On the GPU live path a slider or mask edit mutates `EditSession.model`, whose `didSet` schedules the fast phase as today. Registering a Vision-generated mask raster is a separate, one-time call (`maple_mask_raster_register`, §5.3) ahead of the tick — it needs no new `MapleGpuLiveParams` tail field, since a bitmap layer's `raster_id` already rides the struct's existing `local_adjustments_ptr/len` flat wire and `raw-ffi` resolves it against the registry on every call. `presentViaGpuLive` builds `MapleGpuLiveParams` with the new tail fields of §5.5 (the scope-target layer index, the scope-enabled flag, and a pointer to a stats struct) and calls `maple_gpu_present_chain`. Inside `raw-gpu`:
...
The flat record grows from 24 to 32 floats, two more `vec4`s: slots 24…30 carry `range_kind` (0 none, 1 colour) and the six parameters. `LAYER_FLAT_LEN`, the WGSL `Layer` struct, `flat.rs`'s slot map, and the C# mirror change in the same PR (gate #3221). On the wire the refinement is seven `papp:Range*` attributes on the correction's `rdf:Description` (`papp:RangeKind` plus the six numeric fields), Maple-private by design: Adobe's own range-mask schema is undocumented and a reference renderer that ignores the attributes still applies the correction through the primary mask, which is the graceful degradation.

### 5.3 Bitmap and whole-image masks

Two new variants. `Mask::Bitmap { recipe: BitmapRecipe, raster_id: u32 }` — `recipe` is the sidecar-carried identity (which person, which sub-regions, which segmentation model, and a 16-hex-char content digest); `raster_id` is a purely in-process handle, never written to XMP, that resolves against the raster registry below. The raster itself is an R8 plane sampled bilinearly in the same oriented, uncropped, normalized coordinate frame linear and radial masks use, so crop and orientation need no special handling. `Mask::Everywhere` is weight 1, the primitive behind "skin range only".

**Refined during implementation:** rasters do NOT ride the per-tick params structs as a table + plane, the design this section originally sketched. That would mean a multi-megabyte copy across the FFI on every slider tick even when the mask itself hasn't changed — the params structs already carry the geometry-only layer stack precisely because that stays cheap at 32 floats/layer, and a raster is orders of magnitude larger. Instead the host registers each raster ONCE — `maple_mask_raster_register(digest, width, height, data) -> id` / `maple_mask_raster_release(id)` — in a process-wide registry (`raw-ffi/src/mask_registry.rs`), and every entry family resolves ids from that one table: `xmp::parse` resolves by `papp:MaskDigest` right after parsing (`resolve_into`), while the per-tick CPU-chain and GPU-live params (which carry only the flat wire, no XMP) resolve by the `raster_id` already embedded in a bitmap record's slot 2 (`layers_and_rasters_from_flat`). A `raster_id`/digest with no match in the registry resolves to weight 0 — the correction never silently becomes global.

On the GPU the resolved rasters ride a FOURTH storage buffer per dispatch — the concatenated "mask plane," built fresh by `LocalAdjustmentsPass::new` each call, which also rewrites a bitmap record's geometry slots from `(width, height, raster_id)` to `(width, height, plane_offset)` for the copy it uploads. That puts `local_adjustments` at the four-storage-buffer `downlevel_defaults()` ceiling (params uniform + src + dst + layers + plane), with no headroom for a fifth binding.
...
Rasters cache at `.maple/masks/<sha16(basename)>-<recipeDigest>.png`, the thumbnail cache's naming rule, where the digest hashes the recipe attributes and the model identifier. `EditSession` owns a `MaskRasterStore` that resolves every `Mask::Bitmap` recipe in the model to a raster and calls `maple_mask_raster_register` once per CHANGE, not per tick — the returned id is cheap to carry on every subsequent render call, so a slider drag never re-uploads the raster (§5.3's registry design). Opening a sidecar whose raster is missing regenerates it in the background; until it lands the layer renders at weight 0 and its row shows "Regenerating mask"; if regeneration fails the row shows "Mask unavailable" and the layer stays inert.
