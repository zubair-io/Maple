...
final class NonRawSupportTests: XCTestCase {
    ...
    /// Regression for #1604: a sourceless / bytes-backed asset (Self-Hosted,
    /// SMB, PhotoKit) must seed `nativeImageSize` from its bytes. Cold-open
    /// (`ensureRenderStarted`) relies on this so the canvas frame resolves and
    /// the GPU live layer registers BEFORE the decode lands — without an early
    /// seed a remote asset stays placeholder-only and falls to the CPU path.
    @MainActor
    func testSeedsNativeImageSizeFromBytesProvider() async {
        ...
        XCTAssertEqual(
            session.nativeImageSize, CGSize(width: w, height: h),
