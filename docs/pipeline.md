...
Web display scopes (#3397) consume sRGB RGB samples for all four plots, including per-column waveform/parade data that the native Cb/Cr histogram does not contain. `ScopeSampler` samples the final resident buffer at pixel centers with a 512px long-edge cap, using the presentation shader's quantizer and original pixel coordinates for dither. These are actual quantized pixels rather than the previous canvas downsample's interpolated averages. GPU buffers, bind groups and pipelines are session-owned. `WebLiveSession.sample_scope()` synchronously submits a copy and returns an independently owned map Promise; the mutable render queue never awaits that Promise. A staging buffer cannot be reused until its map is released; failed maps retire that sampler until the session reopens.

Each render submits presentation and awaits the configured browser queue's completion before acknowledging success. This keeps GPU admission bounded while the existing editor scheduler coalesces pending edits. Queue completion includes preceding native maps; scope JavaScript delivery and conversion remain independent, and neither the fence nor the reply establishes compositor scanout timing. The worker publishes completed samples on a separate `session-scope` event. One map runs at a time; newer edits coalesce into a trailing capture, so the final edit and a single discrete edit both refresh the plots without another render. Session identity and monotonic render IDs prevent an old image or older sample from replacing newer scopes, while allowing the newest completed sample to remain visible during a drag. Display P3 samples are converted to sRGB in a CPU-only canvas; the live WebGPU canvas is never synchronously read for production scope updates.
