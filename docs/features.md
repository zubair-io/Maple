...
Apple's editor uses **one control layout** on Mac, iPad and iPhone: `StackedAdjustmentsPanel` shows all four groups in one scrollable inspector beside `ToolDock`. `MapleLayout` moves the same inspector above a horizontally scrolling dock at compact width. Dock entries expand and reveal their group or tool; Crop, Tone Curve, Film, HSL, the eight-band black-and-white mix, Color Grading and Lens Corrections remain reachable in the shared panel. The controls stay visible, and resizing or rotating repositions them around the same edit session, armed tool, undo history and zoom state. There is no layout setting. Its tool enum (`MapleCore/Editor/ToolModel.swift`) matches web's 25 tools and adds three more — a first-class Tone Curve tool and the two capture-sharpening (deconvolution) controls that web does not expose. Chrome includes a mini histogram, an AUTO button, a before/after toggle that appears only once the image is dirty, and a zoom-percent readout.
