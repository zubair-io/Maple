...
enum MaskSlider: String, CaseIterable, Identifiable {
  case hue, temperature, tint, saturation, vibrance, exposure, contrast, highlights, shadows,
    whites, blacks
...
private struct MaskSliderRow: View {
  ...
  private var value: Double {
    guard let layer = state.session.model.localAdjustments.first(where: { $0.id == layerId }) else {
      return 0
    }
    ...
    HStack {
      ...
      Slider(
        value: Binding(
          ...
          set: { newValue in
            guard
              let idx = state.session.model.localAdjustments.firstIndex(where: { $0.id == layerId })
            else { return }
