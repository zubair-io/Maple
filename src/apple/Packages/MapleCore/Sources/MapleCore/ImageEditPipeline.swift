...
public actor ImageEditPipeline {
  ...
  nonisolated public func decode(
    ...
    do {
      ...
      logger.error(
        "decode failed for \(asset.displayName, privacy: .public): \(error.localizedDescription, privacy: .public)"
      )
      return nil
    }
  ...
  public struct SceneLinearDecodeResult: Sendable {
    ...
    /// comments. Survives the decode→render hand-off via
    /// `DecodedSnapshot` so the lens-correction inspector panel can
    /// disable/hide itself and grey the CA/distortion sliders without
    /// re-decoding.
    ...
    do {
      ...
      if let url = asset.primaryURL {
        ...
        imageData = try PipelineRenderer.renderSceneLinear(
          ...
          profileOverride: profileOverride, autoExposureOverride: autoExposureOverride,
          cancel: cancel
        )
      } else if let provider = asset.bytesProvider {
        let bytes = try await provider()
      ...
      // Cancellation during source I/O is a dropped request, never a
      // reason to retry the same RAW at an unbounded resolution.
      return nil
      ...
      logger.debug(
        "decodeSceneLinear cancelled for \(asset.displayName, privacy: .public) (superseded)")
      return nil
    } catch {
      logger.error(
        "decodeSceneLinear failed for \(asset.displayName, privacy: .public): \(error.localizedDescription, privacy: .public)"
      )
      return nil
    }
  ...
  nonisolated public func decodeSceneLinearSized(
    ...
    do {
      ...
      logger.debug(
        "decodeSceneLinearSized cancelled for \(asset.displayName, privacy: .public) (superseded)")
      return nil
    } catch {
      logger.error(
        "decodeSceneLinearSized failed for \(asset.displayName, privacy: .public): \(error.localizedDescription, privacy: .public). Falling back to unsized scene-linear path."
      )
  ...
  nonisolated public func decodeSceneLinearNonRaw(
    ...
    if let url = asset.primaryURL {
      ...
      do {
        ...
        logger.error(
          "decodeSceneLinearNonRaw bytesProvider failed for \(asset.displayName, privacy: .public): \(error.localizedDescription, privacy: .public)"
        )
        return nil
      }
    } else {
    ...
    guard let cgImage else {
      logger.error(
        "decodeSceneLinearNonRaw: ImageIO returned nil for \(asset.displayName, privacy: .public)")
      return nil
    }
  ...
  nonisolated private func applySceneLinearChainViaFFI(
    ...
    let wrapped: CIImage? = autoreleasepool {
      ...
      do {
        ...
        logger.error(
          "applySceneLinearChainViaFFI: FFI error: \(error.localizedDescription, privacy: .public)")
        return nil
      }
  ...
  nonisolated private func applyChainAndEncodeViaFusedFFI(
    ...
    return autoreleasepool {
      ...
      do {
        ...
        logger.error(
          "applyChainAndEncodeViaFusedFFI: FFI error: \(error.localizedDescription, privacy: .public)"
        )
        return nil
      }
  ...
  nonisolated private func encodeDisplayViaFFI(
    ...
    return autoreleasepool {
      ...
      do {
        ...
        logger.error(
          "encodeDisplayViaFFI: FFI error: \(error.localizedDescription, privacy: .public)")
        return nil
      }
  ...
  nonisolated public func processSceneLinear(
    ...
    if let asShot {
      logger.notice(
        "processSceneLinear asShot=\(asShot.temperature, format: .fixed(precision: 0))K/\(asShot.tint, format: .fixed(precision: 1)) live=\(model.temperature, format: .fixed(precision: 0))K/\(model.tint, format: .fixed(precision: 1)) → decodedTemp=\(decodedTemp, format: .fixed(precision: 0))"
      )
    } else {
      logger.notice(
        "processSceneLinear asShot=NIL live=\(model.temperature, format: .fixed(precision: 0))K/\(model.tint, format: .fixed(precision: 1)) → decodedTemp=\(decodedTemp, format: .fixed(precision: 0))"
      )
    }
  ...
  nonisolated static func prescaleForDisplay(
    ...
    let lanczos = clamped.applyingFilter(
      "CILanczosScaleTransform",
      parameters: [
        kCIInputScaleKey: scale,
        kCIInputAspectRatioKey: 1.0,
      ])
    return lanczos.cropped(
      to: CGRect(
        x: 0, y: 0,
  ...
  nonisolated public func decodePreviewTile(
    ...
    guard srcRect.width > 0, srcRect.height > 0,
      outSize.width > 0, outSize.height > 0
    else {
      return nil
    }
    let imageData: MapleSceneLinearImageData
    do {
      ...
      logger.error(
        "decodePreviewTile failed for \(asset.displayName, privacy: .public): \(error.localizedDescription, privacy: .public)"
      )
      return nil
    }
    let w = imageData.width
