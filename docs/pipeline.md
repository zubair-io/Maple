...
| 3   | `lateral_ca`                                | `stages/lateral_ca.rs`                                         | raw mosaic domain                                                              |
| 4   | `demosaic`                                  | `demosaic/{half_res,bilinear,rcd,hamilton_adams,amaze,xtrans}` | camera-native linear RGB                                                       |
| 5   | `opcode_list3`                              | `pipeline/pano/opcode_apply/`                                  | camera RGB, ActiveArea coords                                                  |
...
| 12a | `retouch` (clone / heal spots)              | `stages/retouch/`                                              | scene-linear Rec.2020                                                          |
...
| 26  | `defringe`                                  | `stages/defringe.rs`                                           | scene-linear Rec.2020, via Oklab                                               |
| 27  | `local_adjustments`                         | `stages/local_adjustments/`                                    | scene-linear Rec.2020                                                          |
| 28  | `vignette`                                  | `stages/vignette.rs`                                           | scene-linear Rec.2020                                                          |
| 29  | `sharpen`                                   | `stages/sharpen.rs`                                            | scene-linear Rec.2020, luma-only USM                                           |
| 30  | `nr_luminance`                              | `stages/noise_reduction.rs` → `stages/nlm.rs`                  | Oklab L plane                                                                  |
| 31  | `nr_color`                                  | same                                                           | Oklab a/b planes                                                               |
...
**Highlight recovery.** `HighlightRecoveryMode` defaults to `ChromaticAdaptation`, which runs pre-DCP in camera RGB where the per-channel ceiling after the WB pre-gain is `1/AsShotNeutral[c]` rather than 1.0 — the reason a naive clip reads magenta. `OklabChromaReduction` is an opt-in variant that instead runs post-DCP in scene-linear Rec.2020, scaling Oklab `a`/`b` by a common factor so hue is preserved by construction. `Blend` and `Luminance` are legacy XMP values silently upgraded to `ChromaticAdaptation`.
...
9. **Geometry tail** — `pipeline/render/finish.rs` applies EXIF orientation, then the manual-geometry homography, then the user crop, depth-generically, so the canvas render and the export master frame identically.

The manual-geometry stage (`stages/perspective`, #3410) composes Adobe's seven `crs:Perspective*` scalars — vertical and horizontal keystone, rotation, scale, area-preserving aspect stretch, and X/Y offset — into ONE 3×3 homography and applies it in ONE bilinear resample, sharing `stages::crop`'s sampler so the two geometry stages cannot disagree about the out-of-bounds fill or the narrowing round. It works in centred, half-extent-normalized coordinates (`[-1, 1]` with `y` down), which is what makes every slider resolution-independent and lets the same matrix serve the CPU tail and the WGSL present shader; only the rotation factor re-introduces the pixel aspect ratio, conjugated by `diag(ar, 1)` so it stays circular on a non-square frame. Composition order is `T · S · A · R · P` — keystone about the centre, then rotation, then the aspect stretch, then uniform scale, then the offset — and the three constants that pin what one slider unit buys (`KEYSTONE_MAX`, `ASPECT_MAX_RATIO`, `OFFSET_MAX`) live in `stages/perspective/matrix.rs`, because Adobe does not document its own mapping. Every field at its default composes to the exact identity matrix, which the stage short-circuits before any allocation, so an image with untouched geometry renders bit-identically to a build without the stage. Output dimensions are unchanged: pixels pulled from outside the source render black, as Adobe renders them, and the user frames them out with the crop that runs immediately after — auto-constrain is deliberately not part of this stage.

Orientation runs first because the sliders are authored against what the photographer sees, not against sensor framing; the crop runs last because the crop rect is how the surround gets framed out, so it has to be able to see it.
...
**Stage 27 carries two groups.** A local-adjustment layer's eleven POINT controls are per-pixel operators scaled by the mask weight, evaluated in one pass over the buffer. Its six SPATIAL controls (#3407 — texture, clarity, dehaze, sharpness, luminance noise, defringe) read a neighbourhood, so there is no weighted per-pixel form: `stages/local_adjustments/spatial.rs` runs the GLOBAL stage's own kernel over a scratch copy of that layer's output at the layer's slider value and lerps the result back by the mask weight (`out = base + w · (stage(base) − base)`). At `w = 1` that is bit-for-bit the global stage; at `w = 0` it is identity, and a layer that engages none of the six never allocates the scratch. `stages/defringe.rs` serves this group AND stage 26 above (#3411 added the global ACR controls): ONE kernel, whose parameters are a superset — the per-mask caller passes a single hue-agnostic amount, the global caller two hue-banded ones, and a pixel takes the strongest claim on it. With no band engaged the kernel never evaluates a hue, so the per-mask path is bit-identical to the single-amount stage it replaced. On the GPU the same structure is a per-layer split: when some layer engages the group, `raw-gpu`'s live chain emits one pass per layer instead of the fused single dispatch, and the engaged layer's pass drives the shipping clarity / texture / dehaze / sharpen / NLM / defringe kernels over scratch before blending by the weight `local_adjustments.wgsl` already writes into alpha for the vectorscope.

`pipeline/scene_linear_chain.rs` is what the editor actually calls on a slider tick when the GPU path is unavailable. It takes an already-decoded fp16 or f32 RGBA scene-linear Rec.2020 buffer and re-applies only the model-dependent stages: `white_balance::apply_delta` → `scene_tone_controls` → `tone_curves` → `vibrance` → `saturation` → `hsl` → `clarity` → `texture` → `dehaze` → `defringe` → `local_adjustments` → `vignette` → `sharpen` → `nr_luminance` → `nr_color` → `agx` → `color_grade` → `grain` → display encode. White balance is applied as a _delta_ against the temperature/tint the buffer was decoded at (carried in `ChainOptions`), so opening a saved sidecar does not double-apply WB; when the decode exported a `SliderFrameExport`, the delta is derived in that same camera-calibration frame, which is what closes the live-vs-refine seam. `ChainOptions::skip_agx` turns off the whole display tail for non-RAW inputs (a JPEG/HEIF already carries a baked tone curve).
...
`pipeline/tile/` linearizes only a padded crop region and runs a stripped develop chain on it, then trims the overlap and packs oriented fp16/f32 RGBA. Auto-exposure is never recomputed per tile (a tile's histogram is not the scene's) — the caller threads in the gain the full-image develop measured. `TILE_OVERLAP_PX` is pinned by a const-assert to clarity's guided-filter reach (`2 × CLARITY_GUIDED_RADIUS`); stages whose stencil does not fit — dehaze, vignette, deep denoise, local adjustments, capture sharpening — are rejected loudly at the entry rather than rendered wrong. `retouch` (#3409) is rejected per SPOT rather than per model: a repair spot reads a whole neighbourhood and its source can sit anywhere in the frame, so a tile holding a spot's destination but not its whole footprint refuses; a tile no spot reaches renders normally. See [zoom](zoom.md).
...
Ported stages, each parity-gated directly against its `raw-core` function rather than a hand-copied oracle: capture sharpening, white balance, scene tone controls, tone curves, vibrance, saturation, HSL, clarity, texture, dehaze, defringe, local adjustments, vignette, sharpen, NLM luma, NLM colour, colour grade, film LUT, grain, AgX, display encode, sRGB gamma, Auto Profile curve, residual LUT, and the terminal dither. Auto exposure stays on the CPU. Spatial stages (clarity/texture/dehaze/NLM/sharpen/capture sharpening) stay a single `Pass` each but orchestrate a small DAG or a dispatch loop over scratch planes via the shared `spatial.rs` primitives, because the WGSL downlevel baseline allows only four storage buffers per kernel. `local_adjustments` sits AT that same ceiling too (#3271): a `Mask::Bitmap` layer's raster pixels ride a fourth storage buffer (the concatenated "mask plane", alongside the src/dst/layer-stack buffers), leaving no headroom for a fifth.

Capture sharpening is the one ported stage that does not run per tick on any shipping front end. It sits at develop's stage-14 position, upstream of everything the live chain owns, so Apple, web and Windows all bake it into the DECODE PRODUCT and pass `capture_sharpening_enabled = 0` / `capture_sharpening: None` to the chain; moving either `captureSharpeningAmount` or `captureSharpeningSigma` invalidates that product and re-develops once, on gesture release. `CaptureSharpeningPass` is what runs when a caller does gate it on — it is the FIRST pass in both `full_chain.rs` and `live_chain.rs` (signature bit 0), so the same WGSL kernel serves a GPU-resident decode product without a double apply. Its Richardson–Lucy loop is encoded as a per-iteration dispatch loop over scratch planes, parity-gated at `< 1e-4` directly against `apply_capture_sharpening`.
...
The present shader also carries the manual-geometry warp (#3410): when a `PresentGeometry` is armed it resamples the chain buffer through the destination→source homography instead of loading a pixel straight, reproducing raw-core's _hybrid_ out-of-bounds rule exactly — a footprint fully outside the source is opaque black, one still straddling the border clamps its individual taps — because plain clamp-to-edge would leave a one-pixel bright fringe around every warped frame. The matrix is built by raw-core and carried into `raw-gpu` as three `vec4` rows plus an active flag, since the crate dependency runs the other way and a second copy of the math is exactly what would drift. It is deliberately NOT part of the `PresentDispatchCache` key — a slider drag changes it every tick, and keying on it would turn every tick into a cache miss — so the uniform is rewritten per present instead, which allocates nothing. A zeroed flag takes the untouched pre-#3410 load paths byte for byte, and `present_chain/tests.rs` gates all three: the warp against raw-core's own f32 warp at ≤ 1 LSB, the identity case as byte-identical, and the surround as black rather than smeared.
