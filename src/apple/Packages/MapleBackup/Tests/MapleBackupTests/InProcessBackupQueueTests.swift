...
final class InProcessBackupQueueTests: XCTestCase {
  ...
  func testEnqueueAndDequeueInPriorityOrder() async throws {
    ...
    await q.enqueue(
      BackupTask(
        id: BackupTaskID(deviceId: "d", phassetLocalId: "low"),
        state: .pending, priority: .background),
      priority: .background)
    await q.enqueue(
      BackupTask(
        id: BackupTaskID(deviceId: "d", phassetLocalId: "hi"),
  ...
  func testFIFOWithinSamePriority() async throws {
    ...
    for name in ["A", "B", "C"] {
      await q.enqueue(
        BackupTask(
          id: BackupTaskID(deviceId: "d", phassetLocalId: name),
          state: .pending, priority: .background),
        priority: .background)
    }
    let order = [
  ...
  func testCancelRemovesPendingTask() async throws {
    ...
    await q.enqueue(
      BackupTask(id: id, state: .pending, priority: .background),
      priority: .background)
    await q.cancel(id)
  ...
  func testSnapshot() async throws {
    ...
    await q.enqueue(
      BackupTask(
        id: BackupTaskID(deviceId: "d", phassetLocalId: "A"),
        state: .pending, priority: .background),
      priority: .background)
    await q.enqueue(
      BackupTask(
        id: BackupTaskID(deviceId: "d", phassetLocalId: "B"),
        state: .pending, priority: .userEdit),
      priority: .userEdit)
  ...
  func testObserveEmitsEnqueueEvent() async throws {
    ...
    Task {
      await q.enqueue(
        BackupTask(
          id: BackupTaskID(deviceId: "d", phassetLocalId: "y"),
          state: .pending, priority: .background),
        priority: .background)
    }
...
final class InProcessBackupQueueCaptureOrderTests: XCTestCase {
  private func task(_ name: String, capturedAt: Date?, priority: BackupPriority = .background)
    -> BackupTask
  {
    BackupTask(
      id: BackupTaskID(deviceId: "d", phassetLocalId: name),
  ...
  func testFreshCaptureJumpsAheadOfOlderBacklog() async throws {
    ...
    let base = Date(timeIntervalSince1970: 1_700_000_000)
    await q.enqueue(
      task("backlog-1", capturedAt: base.addingTimeInterval(-3600)), priority: .background)
    await q.enqueue(
      task("backlog-2", capturedAt: base.addingTimeInterval(-7200)), priority: .background)
  ...
  func testUndatedTasksSortAfterDatedOnesThenFIFO() async throws {
    ...
    await q.enqueue(
      task("dated", capturedAt: Date(timeIntervalSince1970: 1)), priority: .background)
  ...
  func testPriorityStillOutranksCaptureDate() async throws {
    let q = InProcessBackupQueue()
    await q.enqueue(
      task("newest-background", capturedAt: Date(timeIntervalSince1970: 9_999_999)),
      priority: .background)
    await q.enqueue(
      task("old-user-edit", capturedAt: Date(timeIntervalSince1970: 1), priority: .userEdit),
      priority: .userEdit)
    let first = await q.dequeue()
    XCTAssertEqual(first?.id.phassetLocalId, "old-user-edit")
  }

  func testHundredThousandPhotosDrainNewestFirstWithoutDuplicates() async {
    let queue = InProcessBackupQueue()
    let start = Date()
    for i in 0..<100_000 {
      let task = BackupTask(
        id: BackupTaskID(deviceId: "scale", phassetLocalId: "\(i)"),
        state: .pending, priority: .background,
        capturedAt: Date(timeIntervalSince1970: Double(i)))
      await queue.enqueue(task, priority: .background)
    }
    let repeated = BackupTask(
      id: BackupTaskID(deviceId: "scale", phassetLocalId: "50000"),
      state: .pending, priority: .background,
      capturedAt: Date(timeIntervalSince1970: 50000))
    await queue.enqueue(repeated, priority: .background)
    await queue.cancel(BackupTaskID(deviceId: "scale", phassetLocalId: "40000"))
    for i in (0..<100_000).reversed() where i != 40000 {
      let task = await queue.dequeue()
      XCTAssertEqual(task?.id.phassetLocalId, "\(i)")
    }
    let extra = await queue.dequeue()
    XCTAssertNil(extra)
    print("100k backup queue enqueue/drain: \(Date().timeIntervalSince(start)) seconds")
  }
}
