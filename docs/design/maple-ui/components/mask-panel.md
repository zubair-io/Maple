...
- `state` (Apple) / `session` (web): the editor state carrying the selected layer index and the
  live model.
...
- Writes: through the editor state's mask API only — `addLinearMask`, `addRadialMask`,
  `removeMask(at:)`, `selectMask`, `setMaskAdjustment`, `setSelectedMaskFeather`,
  `setSelectedMaskInverted`, `resetSelectedMaskAdjustments`.
...
- The panel is a container element with identifier `editor-mask-section`.
