 ...
 * per-entry cooldown, recover/prune + re-arm, mount guard (offline + empty
 * root), name-mismatch veto, circuit breaker, pagination past a clogged
 * batch, and pause-skips-delete-but-still-recovers. Orphan reaping,
 * claim-query parking, and the start/pause/resume control surface live in
 * `missing-reaper.control.test.ts` (split for the file-size budget).
 */
...
const TEST_DB = withTestDb(`maple_test_missingreaper_${process.pid}`);
