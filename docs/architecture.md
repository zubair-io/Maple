...
| **Maple API** (Self Hosted server)               | `src/api/`                       | `bun src/index.ts`, Docker or systemd               | Nothing external; optionally Meilisearch, Cloudflare R2, Nominatim, a describe model server |
...
4. **The library database.** One SQLite file — named by `MAPLE_SQLITE_PATH`, default `./data/maple.sqlite` — holding the server's searchable index: one row per file, carrying EXIF, geocoded place, faces, description text, and per-stage progress. Explicitly non-authoritative: `src/api/src/db/schema.ts` says so in its header. Tables include `folders`, `assets`, `people`, `jobs`, `imports`, `indexer_queue`, `discover_frontier`, `asset_changes`, `mirror_queue`, and the auth set (`users`, `credentials`, `invites`, `refresh_tokens`, `challenges`). See [sqlite-schema](sqlite-schema.md).
...
| Camera / lens support tiers                 | `support_tiers` + the bundled profile index + the same evidence records | Swift (`MapleCore/Generated/CameraSupport+Generated.swift`), TS (`maple-common/src/lib/generated/camera-support.generated.ts`), `docs/camera-support.{md,json}` — tiers computed, never asserted |
