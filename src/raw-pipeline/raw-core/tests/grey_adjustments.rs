...
// grain / split-tone stages, #1109–#1111) live in the sibling target
// `grey_adjustments_display.rs` — split out to stay under the 600-LOC file
// budget (#1170).
...
fn dump_scene_linear_deltas() {
    fn worst_delta(
        ...
        let mut model = AdjustmentModel::default();
        configure(&mut model);
        let img = develop_scene_linear_from_raw_with_quality(
            &raw, &model, RenderQuality::Full).unwrap();
    ...
    fn dump(label: &str, delta: f32, predicted: f32) {
        let rel = if predicted.abs() > 1e-9 { delta / predicted.abs() } else { f32::NAN };
        println!("{:32} |Δ|={:.3e}  (predicted={:.4}, rel={:.3e})", label, delta, predicted, rel);
    }
...
// ---- HSL 8-band neutral pass-through (#1112) --------------------------------
//
...
fn hsl_all_hue_max_is_identity_on_neutral() {
    ...
    for L in [0.05_f32, 0.18, 0.50, 0.80] {
        assert_predicted_scene_linear(
            ...
            |s| s, // identity — chroma gate fires
        );
        assert_neutral_display(L, |m| {
            m.hue_adjustment_red      = 100.0;
            m.hue_adjustment_orange   = -100.0;
