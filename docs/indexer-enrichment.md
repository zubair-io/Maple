...
**`describe` is locked in code.** Provider, model, and prompt are not operator-configurable — `DESCRIBE_VISION_OLLAMA_TAG` (`gemma4:12b`) and `DEFAULT_DESCRIBE_VISION_PROMPT` / `DESCRIBE_VISION_PROMPT_VERSION` in `enrichment/`. The structured-JSON parser only accepts the shape that prompt plus Ollama's grammar-constrained decoding produces, so an operator override would dead-letter every row. What _is_ configurable is the list of Ollama server URLs, pooled by `enrichment/describe-server-pool.ts` with per-server concurrency, failover, and a per-server circuit breaker: after two consecutive server-fault failures (network error, timeout, 5xx — Ollama's model runner crashing mid-generation is the canonical case) a server sits out for 30 s, and a call with no other server to go to waits for that cool-down and runs as the single probe instead of failing. Only the calls in flight when the runner died spend an attempt; the backlog claimed during the reload is held rather than dead-lettered. The Ollama provider carries the server's own `{ "error": … }` body into the error message, so `last_error` names the actual reason (`model runner has unexpectedly stopped …`) rather than a bare status. OCR is not a separate engine: `ocr_text` is mirrored from the vision model's `text_visible`, and `ocr_meta.engine` is the literal `"qwen2.5-vl"`. Because the on-disk preview is AVIF and every provider sends `image/jpeg`, the handler re-encodes to JPEG in memory per call rather than persisting a second artefact.
