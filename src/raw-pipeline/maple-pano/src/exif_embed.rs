...
impl Le {
    ...
    /// Overwrite a u32 at a previously reserved position.
    fn patch_u32(&mut self, pos: usize, val: u32) {
        self.0[pos..pos + 4].copy_from_slice(&val.to_le_bytes());
    }
...
pub fn build_exif_blob(exif: &Exif) -> Option<Vec<u8>> {
    ...
    let n_ifd0_ptr = (!exif_entries.is_empty() as usize) + (!gps_entries.is_empty() as usize);
