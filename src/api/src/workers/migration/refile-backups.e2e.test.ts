...
describe("refile-backups end-to-end", () => {
  ...
  it("moves a geocoded backup into year/Country/City, stamps v3, resets cache, idempotent", async () => {
    ...
    try {
      ...
      expect(
        await fs.readFile(
          path.join(dir, "2024/Japan/Kyoto/IMG_GEO.HEIC"),
          "utf8",
        ),
      ).toBe("pixels");
      expect(
        await fs.readFile(
          path.join(dir, "2024/Japan/Kyoto/IMG_GEO.xmp"),
          "utf8",
        ),
      ).toBe("edits");
  ...
  it("THE regression: unfreezes a backup_layout_version:2 asset stuck at a stale POI path", async () => {
    ...
    try {
      ...
      expect(
        await fs.readFile(
          path.join(dir, "2026/France/Paris/IMG_0333.JPG"),
          "utf8",
        ),
      ).toBe("pixels");
      await expect(fs.stat(path.join(dir, oldRel))).rejects.toThrow();
  ...
  it("moves a flagged screenshot into year/Screenshot", async () => {
    ...
    await fs.mkdir(path.join(dir, ...oldRel.split("/")), { recursive: true });
    await fs.writeFile(
      path.join(dir, oldRel, "Screenshot 2024-03-15.png"),
      "pixels",
    );
    ...
    try {
      ...
      expect(
        await fs.readFile(
          path.join(dir, "2024/Screenshot/Screenshot 2024-03-15.png"),
          "utf8",
        ),
      ).toBe("pixels");
    } finally {
  ...
  it("flattens an old MM-DD day-folder for a non-geo backup", async () => {
    ...
    try {
      ...
      expect(
        await fs.readFile(path.join(dir, "2024/Misc/IMG_E2E.HEIC"), "utf8"),
      ).toBe("pixels");
      await expect(fs.stat(path.join(dir, oldRel))).rejects.toThrow();
    } finally {
      await assets.deleteOne({ _id });
  ...
  it("already-correct asset is stamped without moving the file (no-op)", async () => {
    ...
    try {
      ...
      expect(
        await fs.readFile(path.join(dir, rel, "IMG_OK.HEIC"), "utf8"),
      ).toBe("pixels");
    } finally {
      await assets.deleteOne({ _id });
      await resetMigrationState(MIGRATION_ID);
  ...
  it("auto-disables its own toggle when the sweep completes", async () => {
    ...
    const { setLibraryRootsForTests } =
      await import("../../indexer/libraries.cache.ts");
    const { resetMigrationState, loadMigrationState } =
      await import("../migration-config.repo.ts");
    const assets = db.collection("assets");
    const libId = new ObjectId();
