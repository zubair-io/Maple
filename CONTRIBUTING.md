...
| local only  | `python3 tools/check-perf-ratchet.py <fresh-row.json> <committed-row.json>` — #3421 committed per-device benchmark regression check against `test-fixtures/perf/`; never wired into cloud CI (Apple tests aren't cloud-gated at all — see `docs/performance.md`)                                                                                           |
