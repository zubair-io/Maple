...
final class DisplayPreviewPersistTests: XCTestCase {
  ...
  func testCloudSinkBuildsCanonicalPutRequest() throws {
    ...
    XCTAssertTrue(
      url.absoluteString.contains("path=/lib/Photos/2024/IMG%201234.CR2"),
  ...
  func testScheduleThenFlushPersistsOnceAsAVIF() async throws {
    ...
    let asset = AssetRef(
      displayName: "x.dng", hintExtension: "dng",
  ...
  func testNoSinkNoPersist() async {
    let asset = AssetRef(
      displayName: "y.dng", hintExtension: "dng",
      bytesProvider: { throw CancellationError() })
    let session = EditSession(asset: asset)
    XCTAssertFalse(session.previewPersistence.hasDestination)
