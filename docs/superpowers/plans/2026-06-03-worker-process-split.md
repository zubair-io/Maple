...
async function main(): Promise<void> {
  await getDb();
  try { await ensureIndexes(); } catch (e) { log.warn({ err: e instanceof Error ? e.message : e }, 'ensureIndexes failed — continuing'); }
  try { await initOtel(resolveObservabilityConfig(await loadObservabilityConfig())); } catch (e) { log.warn({ err: e }, 'otel init failed'); }
  await startWorkers();
  log.info('worker tier started');
  const shutdown = async () => { try { await stopWorkers(); } catch { /* best effort */ } await closeDb(); process.exit(0); };
  process.on('SIGTERM', () => void shutdown());
  process.on('SIGINT', () => void shutdown());
}
main().catch((e) => { process.stderr.write(`[worker-main] fatal: ${e instanceof Error ? e.message : e}\n`); process.exit(1); });
```
