...
describe("library-relocate end-to-end", () => {
  ...
  it("relocates a photo + its .xmp sidecar into year/state/city, repoints DB, removes source", async () => {
    ...
    try {
      ...
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_1.dng"), "utf8"),
      ).toBe("pixels");
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_1.xmp"), "utf8"),
      ).toBe("edits");
      // Sources gone.
      await expect(
        fs.stat(path.join(dir, oldRel, "IMG_1.dng")),
      ).rejects.toThrow();
      await expect(
        fs.stat(path.join(dir, oldRel, "IMG_1.xmp")),
      ).rejects.toThrow();
  ...
  it("relocates a photo even if it is flagged missing_since, and clears the missing_since flag in the DB", async () => {
    ...
    await fs.mkdir(path.join(dir, ...oldRel.split("/")), { recursive: true });
    await fs.writeFile(
      path.join(dir, oldRel, "IMG_missing.dng"),
      "missing_bytes",
    );
    ...
    try {
      ...
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_missing.dng"), "utf8"),
      ).toBe("missing_bytes");
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_missing.xmp"), "utf8"),
      ).toBe("edits");
  ...
  it("auto-renames file + sidecar on collision (.N suffix), leaves the pre-existing occupant untouched", async () => {
    ...
    try {
      ...
      // Occupant untouched.
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_2.dng"), "utf8"),
      ).toBe("occupant-pixels");
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_2.xmp"), "utf8"),
      ).toBe("occupant-edits");
      // Moved copy landed at the suffixed sibling (.1) — file AND sidecar.
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_2.1.dng"), "utf8"),
      ).toBe("source-pixels");
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_2.1.xmp"), "utf8"),
      ).toBe("source-edits");
      // Source gone.
      await expect(
        fs.stat(path.join(dir, oldRel, "IMG_2.dng")),
      ).rejects.toThrow();
  ...
  it("excludes videos — a video asset with a place is not counted (#1678)", async () => {
    ...
    try {
      ...
      expect(
        await fs.readFile(path.join(dir, oldRel, "CLIP.mov"), "utf8"),
      ).toBe("frames");
    } finally {
      await assets.deleteOne({ _id });
    }
  ...
  it("relocates to year/Screenshot when metadata_override.is_screenshot is true", async () => {
    ...
    dir = await fs.mkdtemp(
      path.join(os.tmpdir(), "relocate-override-screenshot-"),
    );
    await seedLibrary(libId, dir);
    ...
    try {
      ...
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_scr.dng"), "utf8"),
      ).toBe("pixels");
      expect(
        await fs.readFile(path.join(dir, newRel, "IMG_scr.xmp"), "utf8"),
      ).toBe("edits");
