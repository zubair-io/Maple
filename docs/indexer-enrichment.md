...
| `change-log-gc`      | daily                    | Prunes `asset_changes` — the File Provider change journal — past its retention window (default 30 days, editable on Settings → Workers), in bounded batches with a pause between them so one sweep cannot hold the shared event loop. It raises a monotonic retention floor in `server_state` **before** it deletes anything, so a client anchored inside the doomed range gets the cursor-too-old 409 while those rows are still on disk rather than a page with a silent hole in it; the client then re-enumerates. Freeing the on-disk bytes after a large first sweep still needs a manual `compact` (WiredTiger). |
