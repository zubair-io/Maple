...
- [x] **Step 4: Landing.** The branch was pushed and CI on PR #3740 ran green. The PR description's "What this does" paragraph now describes the hand-rolled hero overlay above the Library stack (the system zoom transition does not run on iOS 27) in place of the earlier zoom-transition sentence, and `docs/features.md`'s iPhone sentence no longer claims that 1-column tiles take each photo's aspect.
