...
import displayPreviewStage, {
  developedPreviewSizeKey,
} from "./display-preview.ts";
...
describe("developedPreviewSizeKey", () => {
  it("formats dev_<ver>.jpg, defaulting undefined to 0", () => {
    expect(developedPreviewSizeKey(3)).toBe("dev_3.jpg");
    expect(developedPreviewSizeKey(0)).toBe("dev_0.jpg");
    expect(developedPreviewSizeKey(undefined)).toBe("dev_0.jpg");
  });
});
...
describe("display-preview handler", () => {
  ...
  it("renders a developed _dev_<ver>.jpg for an edited RAW (fixture + dylib gated)", async () => {
    ...
    const rawLib = await mkdtemp(
      path.join(os.tmpdir(), "display-preview-raw-"),
    );
    try {
      ...
      const result = await displayPreviewStage.handler(
        doc as never,
        {} as never,
      );
      ...
      expect((devPath as string).endsWith("edited.dng.dev_2.jpg")).toBe(true);
