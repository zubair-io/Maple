...
| Historical size exceptions remain        | Gate reports 23 allowlisted paths; e.g. `raw-core/src/color/dcp.rs` 3,189 lines, `FileProviderExtensionCore.swift` 2,607, `PipelineRenderer.swift` 2,006 | Historical exceptions and some split-ticket references are recorded in `tools/budget-allowlist.txt`; raw size alone does not establish a defect |
