...
def write_restore_script(quarantine: str, moves: list[tuple[str, str]]) -> str:
    ...
    Every destination is checked before anything moves. ``mv`` into an existing
    directory nests rather than replaces, so restoring onto a path that has been
    recreated since the run would silently produce a different tree than the one
    we started with. Preflighting the whole set means a conflict aborts with
    nothing moved, rather than leaving a half-restored library behind.
    """
    ...
    lines = [
        ...
        "conflicts=0",
        "",
        "# Preflight: refuse to start if any original path has been recreated.",
    ]
    for original, _ in moves:
        quoted = shlex.quote(original)
        lines.append(f'if [ -e {quoted} ]; then echo "occupied: {quoted}" >&2; conflicts=$((conflicts + 1)); fi')
    lines += [
        "",
        'if [ "$conflicts" -ne 0 ]; then',
        '    echo "" >&2',
        '    echo "Aborting: $conflicts destination(s) already exist; nothing was moved." >&2',
        '    echo "Move or remove the paths listed above, then re-run." >&2',
        "    exit 1",
        "fi",
        "",
        "# Restore.",
    ...
    lines += ["", f'echo "Restored {len(moves)} folders."', ""]
...
def write_manifest(quarantine: str, moves: list[tuple[str, str]]) -> str:
    ...
    Uses ``csv`` rather than raw joins so a path containing a tab or a newline
    (both legal on macOS) is quoted instead of corrupting the row.
    """
    path = os.path.join(quarantine, "manifest.tsv")
    with open(path, "w", newline="") as fh:
        writer = csv.writer(fh, delimiter="\t", lineterminator="\n")
        writer.writerow(["original_path", "quarantined_path"])
