...
final class SceneLinearPipelineTests: XCTestCase {
    ...
    // MARK: - Rust reference fixture (AGX_VERSION 6, per-channel LUT path)
    ...
    /// kernel only**, not from `view::agx::apply`. At AGX_VERSION 6, the
    /// `apply` wrapper added a cross-channel `preform_rolloff` stage (see
    /// `src/raw-pipeline/raw-core/src/view/agx.rs`) which the Swift
    /// mirror `agxPerChannelLUT` does not implement. The Spike 1.2 brief
    /// is about the primary-agnostic per-channel sigmoid mirror, so the
    /// generator example also computes per-channel-only values for
