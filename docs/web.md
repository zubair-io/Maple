...
`projects/maple-common/src/lib/raw-pipeline/raw-pipeline.worker.ts` is a module Web Worker that owns the WASM instance. `RawPipelineService` (same directory) lazily creates it, assigns each request a numeric id, and resolves the matching promise when the reply arrives. Every decode is off the main thread. The worker handles these request kinds: `decode`, `develop-non-raw`, `decode-scene-linear`, `open-session`, `render-session`, `close-session`, `set-film-lut`, `auto-adjust`, `sample-wb`, `export`, `native-detail` / `close-native-detail`, and `register-mask-raster` / `release-mask-raster`.
...
### Bitmap-mask raster registry

A `bitmap` local-adjustment mask (`LocalMask` kind `'bitmap'`, `local-adjustment.ts` — the person/skin selection the Apple app derives from Vision, #3271) stores only its recipe in the sidecar; the raster itself is registered with the worker's WASM instance once and referenced by id. `register-mask-raster` (`raw-pipeline.mask-raster.types.ts`, `RawPipelineService.registerMaskRaster`) transfers an R8 raster (`width * height` bytes, 0 = weight 0, 255 = weight 1) under its 16-hex-char recipe digest and resolves with the raster id; `release-mask-raster` forgets it. On the wasm side (`raw-wasm/src/mask_registry.rs`, the mirror of raw-ffi's `maple_mask_raster_register`) every entry that parses a sidecar — the live session, the one-shot CPU and GPU decodes, the scene-linear entries, native detail, export — resolves each `bitmap` layer's digest against that one table before rendering, so a registered raster applies on every path, and the GPU chain binds it as the mask pass's mask plane. A digest nobody registered renders that layer at weight 0 (never a silent whole-image correction); the rest of the sidecar still loads. The registry is per WASM instance: a restarted worker needs its rasters re-registered. Nothing on the web _produces_ a raster yet — the mask overlay treats a `bitmap` layer as unresolved (no handles, tint weight 0) and lists it read-only in the mask panel; choosing the web segmentation source (server-side via the API's `describe` stack, or an in-browser model) is #3300's remaining slice.
...
Both apps register `ngsw-worker.js` with `registerWhenStable:30000`, disabled in dev mode. The two configs share four asset groups — the app shell (prefetch), the WASM (`/raw_wasm_bg.wasm` and `/pkg/**`, lazy but prefetched on update), fonts, and images. Self Hosted's `ngsw-config.json` adds two data groups the Hosted config deliberately omits: thumbnails (`/api/thumb/**`, `/api/assets/*/thumb`, performance strategy, 1500 entries, 30 days) and film LUTs (`/film-luts/*.mlut`, 12 entries, 30 days — the 100 `.mlut` files in `resources/film-luts/` are copied into the build by both apps' asset globs). Library data APIs are never cached, so the server's own index stays authoritative. `AppUpdateService` (`lib/sw/`) watches for a freshly downloaded version, toasts the user, and hard-navigates onto the new build at the next route change.
