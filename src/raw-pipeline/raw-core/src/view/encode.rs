...
// sRGB piecewise gamma OETF/EOTF pair (`srgb_gamma` / `srgb_degamma`) now
// lives in the sibling `gamma` module (#2683 file-budget split) — this
// re-export keeps every existing `view::encode::srgb_gamma` /
// `view::encode::srgb_degamma` call site compiling unchanged.
pub use crate::view::gamma::{srgb_degamma, srgb_gamma};
...
// Gamut guard (#1942) + tests in a sibling module (600-LOC budget); re-exported.
#[path = "encode_gamut_guard.rs"]
mod encode_gamut_guard;
pub use encode_gamut_guard::gamut_guard_display_encoded_srgb;
...
// Quantize / dither tests live in a sibling file (#2683 file-budget split).
#[cfg(test)]
#[path = "encode_quantize_tests.rs"]
mod encode_quantize_tests;
