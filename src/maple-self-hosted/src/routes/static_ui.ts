 ...
 *   <maple-self-hosted>/../../maple-hosted/dist/maple/browser/
...
function resolveUiDist(): string {
  ...
  // Relative to this file's location: src/routes/ → .. → src/ → .. → maple-self-hosted/
  // Then up two more to the monorepo root: ../maple-hosted/dist/maple/browser/
  return path.resolve(
  ...
  .get("/*", async ({ request, set }) => {
    ...
    if (!_distExists && !(await checkDist())) {
      ...
      return {
        ...
        tip: "Run: cd src/maple-hosted && ng build maple --configuration=production",
        dist_path: UI_DIST,
      };
    }
