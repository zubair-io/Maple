/**
 * missing-reaper tests — orphan (content-replaced) reaping, claim-query
 * parking, and the start/pause/resume control surface. Split from
 * `missing-reaper.test.ts` to keep that file under the size budget; the reap
 * pass (recover/prune/delete, guards, pagination) stays there.
 */
...
const TEST_DB = withTestDb(`maple_test_missingreaper_ctl_${process.pid}`);
...
beforeAll(async () => {
  ...
  if (!mongoReachable) {
    console.log('[missing-reaper.control.test] skipping: MongoDB unreachable');
    return;
  }
  db = mongo!.db(TEST_DB);
