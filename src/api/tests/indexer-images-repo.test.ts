...
beforeAll(async () => {
  ...
  if (!mongoReachable) {
    console.log('[indexer-images-repo.test] skipping: MongoDB unreachable at', MONGO_URI);
    return;
  }
  db = mongo!.db(TEST_DB);
...
describe('upsertByMapleId — skeleton schema', () => {
  it('inserts a fresh asset with all enrichment stages pending and outputs nulled', async () => {
    ...
    const doc = await db!
      .collection('assets')
      .findOne({ 'fileinfo.filename': filename, 'fileinfo.library_id': folderId });
...
describe('upsertByMapleId — re-upsert preservation', () => {
  it('does not clobber enrichment.<stage>.done_at on a second upsert', async () => {
    ...
    await db!.collection('assets').updateOne(
      { 'fileinfo.filename': filename, 'fileinfo.library_id': folderId },
      {
        $set: {
    ...
    expect(after!.place.display_name).toBe('New York State Museum, Albany, NY, USA');
...
describe('read-path defaults — old rows without enrichment', () => {
  it('normaliseEnrichment returns a fully-pending skeleton when the field is missing', async () => {
    if (!mongoReachable) return;
    const { normaliseEnrichment, pendingEnrichment } = await import('../src/db/schema.ts');
