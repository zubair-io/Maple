 ...
 * files carry no real EXIF), destination precedence in buildImportFiles
 * (label override → nearby-asset match → shot-folder fallback → misc
 * default), and sidecar-before-image ordering.
 *
 * EXIF-capture-time-specific behavior and the scan-preview destination
 * fields (defaultDest / nearbyMatchCount / nearbyMatchFolders) live in
 * `scan.capture-time.test.ts`.
 */
