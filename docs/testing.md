...
After the canonical GPU+parallel WASM build and `ng build maple-syrup --configuration production`, run `bun run check:live-scopes /absolute/path/to/100mp.dng dist/maple-syrup/browser srgb` and repeat with `display-p3`. This gate requires an actual Chrome WebGPU adapter and explicit RAW; it never skip-passes a missing fixture. It holds a real scope buffer's map Promise while two actual worker renders complete, releases it and checks the final sample against the same frame's independently captured canvas pixels, verifies exposure changes output, and confirms scope refreshes during a 60Hz drag. Its canvas oracle is enabled only for an untimed correctness frame because Chromium may discard the worker canvas backing store after compositor commit. The timed drag performs no canvas readback. A streamed SHA-256 check confirms the original RAW is unchanged.

The script reports 30 unpaced and 30 paced scalar render replies, render/submit timing, asynchronous map timing, final-scope latency and GPU allocation deltas after warmup. The paced P95 reply must remain within 50ms, with 16ms the product target; every tracked GPU resource allocation count must remain unchanged. These are request/acknowledgement and scope-delivery measurements, not compositor scanout timing. Record the machine, browser, achieved color space and artifact hashes with results; a local pass does not promote camera/color qualification. Geometry-specific measurement remains in the lens/geometry lane until that feature is on main.
