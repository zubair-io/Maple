...
//!   * `Profile` and the legacy `papp:Look` migration (ticket #536)
//!   * `HotPixelSuppressionMode` (ticket #1106)
//!
//! Two subjects moved out again in #376, for the same budget reason: the
//! Detail-panel scalars and `CaptureSharpening` live in `tests_detail.rs`,
//! and the JSON-payload attributes in `tests_payloads.rs`.
