...
function placeFor(
  raw: Parameters<typeof parseNominatimResponse>[0],
  lat = 0,
  lon = 0,
) {
  return parseNominatimResponse(
    raw,
    lat,
    lon,
    1,
...
beforeAll(async () => {
  ...
  if (!mongoReachable) {
    console.log(
      "[search-place-route.test] skipping: MongoDB unreachable at",
      MONGO_URI,
    );
    return;
  }
...
describe("/api/search?placeQuery", () => {
  ...
  it("'Albany NY' matches the Albany asset, not New York City", async () => {
    ...
    const r = await app.handle(
      new Request(
        `http://localhost/api/search?placeQuery=${encodeURIComponent("Albany NY")}`,
        { headers: fmtAuth() },
      ),
    );
  ...
  it("AND-restricts with structured filters (camera=Hasselblad)", async () => {
    ...
    const r = await app.handle(
      new Request(
        "http://localhost/api/search?placeQuery=NY&camera=Hasselblad",
        { headers: fmtAuth() },
      ),
    );
    expect(r.status).toBe(200);
  ...
  it("text-score ranks closer matches first (multi-token query)", async () => {
    ...
    const albIdx = body.results.findIndex(
      (r) => r.filename === "albany-museum.dng",
    );
    const nycIdx = body.results.findIndex(
      (r) => r.filename === "nyc-park.dng",
    );
  ...
  it("backfills search_blob for legacy place rows (idempotent)", async () => {
    ...
    const blob = (after as { place?: { search_blob?: string } }).place
      ?.search_blob;
    ...
    const afterAgain = await db
      ...
      .findOne({ _id: legacyId });
    expect(
      (afterAgain as { place?: { search_blob?: string } }).place?.search_blob,
    ).toBe(blob);
