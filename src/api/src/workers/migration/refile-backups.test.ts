/**
 * Tests for the unified backup-refile cleanup migration.
 *
 *  - computeCanonicalDir: pure path/segment derivation (no DB). Subsumes the
 *    geo, screenshot, and folder-flatten path logic of the three migrations this
 *    replaces, so this suite ports their corpora into one table.
 *  - End-to-end runBatch: seeds backup-origin assets + real temp files and drives
 *    one tick. Skips when MongoDB is unreachable (mirrors the deleted migration
 *    tests / smoke.test).
 *  - relocateBackupScreenshot (describe-stage hook): preserved behaviour.
 */
...
import {
  computeCanonicalDir,
  relocateBackupScreenshot,
  BACKUP_LAYOUT_VERSION,
} from './refile-backups.ts';
...
describe('computeCanonicalDir — screenshot (subsumes computeScreenshotDir)', () => {
  ...
  test('location-folder screenshot → year/Screenshot', () => {
    expect(computeCanonicalDir(shot('2024/California/San Francisco'))).toBe('2024/Screenshot');
  });
  ...
  test('already in year/Screenshot → no-op', () => {
    expect(computeCanonicalDir(shot('2024/Screenshot'))).toBe('2024/Screenshot');
  });
...
describe('computeCanonicalDir — degenerate', () => {
  ...
  test('missing fileinfo → null', () => {
    expect(computeCanonicalDir({ place: place({ address: { country: 'France' } }) })).toBeNull();
  });
});
...
describe('refile-backups end-to-end', () => {
  ...
  it('THE regression: unfreezes a backup_layout_version:2 asset stuck at a stale POI path', async () => {
    ...
    await assets.insertOne({
      ...
      fileinfo: [{ path: oldRel, filename: 'IMG_0333.JPG', library_id: libId, deleted_at: null }],
      phasset_links: [
        {
          device_id: 'dev',
          phasset_local_id: 'ph',
  ...
  it('moves a flagged screenshot into year/Screenshot', async () => {
    ...
    await assets.insertOne({
      ...
      fileinfo: [
        { path: oldRel, filename: 'Screenshot 2024-03-15.png', library_id: libId, deleted_at: null },
      ],
      phasset_links: [{ device_id: 'dev', phasset_local_id: 'ph', first_seen: new Date() }],
      is_screenshot: true,
      exif: { captured_year: 2024 },
