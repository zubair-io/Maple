...
final class XMPSerializationTests: XCTestCase {
    ...
    /// above and raw-core's own `"on" | "On"` / `"off" | "Off"` match).
    ...
    /// for the sidecar layer"), mirroring `testSidecarStoreRoundTripKeywords`
    /// above. Regression coverage for #1387: before this ticket the field
    /// didn't exist on the Swift model at all, so a `Profile.neutral` sidecar
