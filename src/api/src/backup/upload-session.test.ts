...
describe('uploadSessions', () => {
  ...
  test('resetAllInProgressBytes zeroes received_bytes on open rows only', async () => {
    const c = await uploadSessionsCollection();
    const now = new Date();
    ...
    await c.insertMany([
      {
        _id: openMid, library_id: libraryId, device_id: deviceId,
        phasset_local_id: 'open-mid', target_rel_path: 'a',
        total_bytes: 1000, received_bytes: 400, chunk_size: 100,
        state: 'open', created_at: now, updated_at: now,
      } as any,
      {
        _id: openFull, library_id: libraryId, device_id: deviceId,
        phasset_local_id: 'open-full', target_rel_path: 'b',
        total_bytes: 1000, received_bytes: 1000, chunk_size: 100,
        state: 'open', created_at: now, updated_at: now,
      } as any,
      {
        _id: openZero, library_id: libraryId, device_id: deviceId,
        phasset_local_id: 'open-zero', target_rel_path: 'c',
        total_bytes: 1000, received_bytes: 0, chunk_size: 100,
        state: 'open', created_at: now, updated_at: now,
      } as any,
      {
        _id: completed, library_id: libraryId, device_id: deviceId,
        phasset_local_id: 'completed', target_rel_path: 'd',
        total_bytes: 1000, received_bytes: 1000, chunk_size: 100,
        state: 'completed', maple_id: 'm-1',
        created_at: now, updated_at: now,
      } as any,
      {
        _id: abandoned, library_id: libraryId, device_id: deviceId,
        phasset_local_id: 'abandoned', target_rel_path: 'e',
        total_bytes: 1000, received_bytes: 800, chunk_size: 100,
        state: 'abandoned', created_at: now, updated_at: now,
      } as any,
    ]);
  ...
  describe('cross-device coordination via phasset_cloud_id', () => {
    ...
    test('abandons stale peer session and proceeds', async () => {
      const c = await uploadSessionsCollection();
      const staleTime = new Date(Date.now() - CROSS_DEVICE_BUSY_WINDOW_MS - 60_000);
