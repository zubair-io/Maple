// PreviewDestination.swift — iPhone wrapper for the fast Preview surface
// (Fast Preview epic §1/§4).
//
// Sibling of `EditorDestination`. On the Library tab it is drawn by the
// `PreviewHero` overlay (`PhoneTabShell`) with the grid live beneath; on the
// Search tab it is a plain `.navigationDestination` push. Either way it
// hands the current-folder asset list, the browse source, and the session
// cache to `PreviewView` and wires:
//   • back / pull-down → `onClose` / `onPullDownCommitted` (the host closes)
//   • Edit             → push `.edit(asset)` onto the stack (→ EditorDestination)
//   • prev/next        → swap the shown asset in place (no push) and keep the
//                        browse VM's `selectedID` in sync so a later Edit /
//                        return-to-grid lands on the right asset.
//
...
struct PreviewDestination: View {
    ...
    /// See `PreviewView.chromeRevealed`.
