...
// bottom-centre — launches Maple on it, waits for the refine pass, and
...
//
// Same staging pattern as `SidecarSeamUITests` (`test_0002-maple-wb.xmp`).
...
// mask parity is gated in raw-gpu's CPU↔WGSL harness (#1698) and the crop
// remap in `MaskRemapRenderParityTests`.
//
//   xcodebuild test \
...
final class MaskFixtureUITests: XCTestCase {
    ...
    /// desaturated ellipse over the bottom-centre move the mean ΔE well
