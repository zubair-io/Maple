...
**gitignored**, so a fresh clone (or a fresh git worktree) does not have it.
`ng serve`, `ng build`, **and `ng test`** all statically import it from every
`*.worker.ts`, so until it exists the affected command fails with
`Could not resolve "./pkg/raw_wasm"`. Build it once (see
...
`prestart`/`prebuild` npm hooks (the `raw-wasm` script in `package.json`);
`build.sh` no-ops in well under a second once `pkg/` is already current for the
checked-out Rust source, so the hook is cheap on a warm tree. **`npm test` /
`bun run test` has no equivalent `pretest` hook** — in a fresh clone or
worktree, `ng test` fails at the same `Could not resolve "./pkg/raw_wasm"`
bundling step `ng serve`/`ng build` do, and nothing rebuilds it for you. Run
the two commands above by hand first if you're only running the test suite:

```bash
cd src/raw-pipeline/raw-wasm && bash build.sh
cd ../../web && bash scripts/sync-raw-wasm.sh
bun run test Maple-common               # or: bun run test maple / maple-syrup
```
...
> build, or test bug (`ng serve`, `ng build`, and `ng test` all fail the same
> way). Rebuild it from **this checkout's own** `raw-wasm` source with the two
> commands above; never copy a `pkg/` from another worktree, since one built
> against a different revision can lack exports the current code imports
> (e.g. `extract_embedded_preview`, added after some other worktree's last
> build). See #2099, #2256.
