...
`managed_certificates` holds the ACME account key, the issued LAN certificate
and any DNS-01 challenge records still outstanding. Leaving it behind means the
server re-issues from scratch on first boot — a new account key, a new
certificate against Let's Encrypt's duplicate-certificate rate limit, and any
TXT records the old challenges created left orphaned in Cloudflare. Recoverable,
but an operator should expect it rather than discover it.
