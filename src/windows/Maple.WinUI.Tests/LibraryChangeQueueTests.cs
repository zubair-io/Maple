...
//
// The two cases below pin down a within-one-debounce-window regression
// found in PR review: a file created then renamed before the debounce
// timer fired was queued as an add of the new name and then immediately
// un-queued by an unconditional cleanup step, so it never reached the UI
// at all; and a file renamed then deleted before the timer fired reported
// the removal under the NEW name — a name the grid, and any sidecar move,
// never actually knew the file by — instead of the original one.
