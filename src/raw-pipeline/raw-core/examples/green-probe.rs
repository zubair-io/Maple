...
fn probe(name: &str, rgb: [f32; 3]) {
    ...
    let clip = [m[0].clamp(0.0,1.0), m[1].clamp(0.0,1.0), m[2].clamp(0.0,1.0)];
    let clip_enc = [srgb_gamma(clip[0]), srgb_gamma(clip[1]), srgb_gamma(clip[2])];
    let luma = |c: [f32;3]| 0.2126*c[0]+0.7152*c[1]+0.0722*c[2];
    println!("=== {name}  in(rec2020-disp-lin)={:?}", rgb);
    println!("  post-matrix sRGB-lin (raw)= [{:.4} {:.4} {:.4}]", m[0],m[1],m[2]);
    println!("  raw-core Oklab-compressed sRGB-lin= [{:.4} {:.4} {:.4}]", lin[0],lin[1],lin[2]);
    println!("  raw-core encoded u8= [{} {} {}]  luma_enc={:.4}", u8v(enc[0]),u8v(enc[1]),u8v(enc[2]), luma(enc));
    println!("  per-chan-clip encoded u8= [{} {} {}]  luma_enc={:.4}", u8v(clip_enc[0]),u8v(clip_enc[1]),u8v(clip_enc[2]), luma(clip_enc));
}
