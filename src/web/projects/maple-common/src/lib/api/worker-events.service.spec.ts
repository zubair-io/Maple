...
describe('WorkerEventsService', () => {
  ...
  it('emits the status payload from a workers-status frame', () => {
    const seen: WorkersStatusResponse[] = [];
    ...
    expect(seen[0].stages[0].name).toBe('hash');
    expect(seen[0].stages[0].pending).toBe(10);
    sub.unsubscribe();
  });
