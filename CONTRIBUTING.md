...
| local only  | `python3 tools/check-perf-ratchet.py <fresh-row.json> <committed-row.json>` — #3421 committed per-device benchmark regression check against `test-fixtures/perf/`; never wired into cloud CI (absolute performance budgets require controlled reference hardware; selected Apple regressions do run in CI — see `docs/performance.md`)                     |
