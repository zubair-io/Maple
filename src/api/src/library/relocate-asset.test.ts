...
async function seedAsset(
  ...
  await d.collection("assets").insertOne({
    _id: id,
    fileinfo: [
      { path: relPath, filename, library_id: libraryId, deleted_at: null },
    ],
    size: 6,
...
describe("relocateAsset", () => {
  test("move: repoints fileinfo, resets thumb/preview + meili, deletes the source", async () => {
    ...
    expect(row.stages.thumb!.version).toBe(0);
    expect(row.stages.thumb!.attempts).toBe(0);
    expect(row.stages.thumb!.last_error).toBeNull();
    expect(row.stages.thumb!.dead).toBe(false);
  ...
  test("collision auto-suffix repoints the DB to the ACTUAL suffixed filename", async () => {
    ...
    const row = (await db
      .collection("assets")
      .findOne({ _id: id })) as unknown as {
      fileinfo: Array<{ filename: string }>;
    };
  ...
  test("a concurrent fileinfo change aborts the repoint and leaves the source untouched (failure direction)", async () => {
    ...
    await db
      ...
      .updateOne(
        { _id: id },
        { $set: { "fileinfo.0.path": "somewhere-else" } },
      );
