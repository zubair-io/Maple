...
That one decode feeds both display phases: the refine (settled) pass directly, and the fast slider-tick pass via a further software 2× box-average downsample (`RenderEngine.DownsampleHalf`). Its own quality is therefore sized as the refine phase — `RefineDecodeQuality.ForTarget` (#3417, mirroring Apple's `ImageEditPipeline.refineDecodeQuality`) escalates the `quality_preview` argument from Preview (a half-res 2×2-binned demosaic) to AMaZE whenever the requested long edge exceeds half the sensor's native long edge, read from the file's EXIF/TIFF IFDs via `Services/Metadata/ExifReader.cs` without a decode. Below that threshold Preview's own cap never engages, so Preview already delivers exactly the requested target and stays in use — this is also what keeps the fast phase implicitly at Preview quality, since it is a software downsample of the same base rather than a second FFI call.
