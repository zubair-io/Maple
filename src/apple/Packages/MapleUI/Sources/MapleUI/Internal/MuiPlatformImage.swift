...
enum MuiPlatformImage {
    /// Decodes `url`'s bytes into a SwiftUI `Image` on a background task.
    /// Returns `nil` on any read/decode failure — never throws, since every
    /// caller here already has its own "broken" visual state to fall into.
    static func load(from url: URL) async -> Image? {
        await Task.detached(priority: .userInitiated) { () -> Image? in
