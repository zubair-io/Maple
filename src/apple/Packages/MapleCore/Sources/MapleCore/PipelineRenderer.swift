...
public struct MapleSceneLinearImageData: Sendable {
  ...
  init(
    ...
    cameraSupport: RawCameraSupport? = nil
  ) {
    self.width = width
    self.height = height
    ...
    self.cameraSupport = cameraSupport
  }
}
...
public struct PipelineRenderer: Sendable {
  ...
  public static func render(
    ...
    try rawPath.withPathCString { rawCStr in
      if let xmpPath {
        return try xmpPath.withPathCString { xmpCStr in
          try _renderWithFilm(
            rawCStr: rawCStr, xmpCStr: xmpCStr, quality: quality, filmLut: filmLut)
        }
      } else {
        return try _renderWithFilm(
          rawCStr: rawCStr, xmpCStr: nil, quality: quality, filmLut: filmLut)
      }
    }
  }
      ...
      try rawPath.withPathCString { rawCStr in
        if let strippedXMP {
          return try strippedXMP.withPathCString { xmpCStr in
            try _renderSceneLinear(
              rawCStr: rawCStr, xmpCStr: xmpCStr, quality: quality, cancel: cancel)
          }
        } else {
          return try _renderSceneLinear(
            rawCStr: rawCStr, xmpCStr: nil, quality: quality, cancel: cancel)
        }
      }
    }
  }
  ...
  private static func _renderSceneLinear(
    ...
    let lastSlash =
      rawPath.lastIndex(of: "/").map { rawPath.index(after: $0) } ?? rawPath.startIndex
    let fileName = String(rawPath[lastSlash...])
    pipelineLog.notice(
      "→ Rust FFI maple_render_file_scene_linear_f32 START: \(fileName, privacy: .public) quality=\(quality.rawValue)"
    )
    ...
    let rc = maple_render_file_scene_linear_f32(
      rawCStr, xmpCStr, quality.rawValue, cancel?.pointer, &buf)
    ...
    let centerOffset =
      (Int(buf.height) / 2) * Int(buf.width) * 16
      + (Int(buf.width) / 2) * 16
    var centerR: Float = 0
    var centerG: Float = 0
    var centerB: Float = 0
    ...
    pipelineLog.notice(
      "← Rust FFI maple_render_file_scene_linear_f32 OK: \(buf.width)x\(buf.height) center scene-linear RGB=(\(centerR, format: .fixed(precision: 4)), \(centerG, format: .fixed(precision: 4)), \(centerB, format: .fixed(precision: 4)))"
    )
    ...
    return MapleSceneLinearImageData(
      ...
      cameraSupport: RawCameraSupport.fromFFI(buf.camera_support_json.map { UnsafePointer($0) })
    )
  }
  ...
  private static func _renderSceneLinearBytes(
    ...
    var buf = MapleSceneLinearBufferF32()
    let rc = hintCStr.withUnsafeBufferPointer { hintPtr -> Int32 in
      maple_render_bytes_scene_linear_f32(
...
extension PipelineRenderer {
  ...
  public static func applySceneLinearChain(
    ...
    guard inputBytes.count == expectedBytes else {
      throw PipelineError.renderFailed(
        code: 9,
        message:
          "applySceneLinearChain: input \(inputBytes.count) bytes != expected \(expectedBytes)"
      )
    }
  ...
  static func withChainPointers<R>(
  ...
  public static func applyChainAndEncodeDisplay(
    ...
    guard inputBytes.count == expectedBytes else {
      throw PipelineError.renderFailed(
        code: 9,
        message:
          "applyChainAndEncodeDisplay: input \(inputBytes.count) bytes != expected \(expectedBytes)"
      )
    }
...
extension URL {
  ...
  fileprivate func withPathCString<R>(_ body: (UnsafePointer<CChar>) throws -> R) throws -> R {
    let p = self.path
