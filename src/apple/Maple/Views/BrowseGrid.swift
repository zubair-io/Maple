...
struct BrowseGrid: View {
  ...
  @Environment(\.batchAdjustmentLibrary) var batchLibrary
  ...
  @State var adjustmentDraft: AdjustmentTransferDraft?
    ...
    VStack(spacing: 0) {
      ...
      if vm.isSelecting, let onMergePanorama {
        PanoSelectionBar(
          ...
          onPasteAdjustments: clipboard != nil ? { pasteAdjustments() } : nil,
    ...
    .sheet(item: $adjustmentDraft) { draft in
      AdjustmentGroupPickerSheet(
        ...
        onApply: { request in
          ...
          Task {
            await controller.start(request: request, targets: draft.targets, library: draft.library)
          }
  ...
  private var normalGrid: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      PhotoGrid(
        ...
        makeItem: { asset in
          PhotoGridItem(
            local: asset, source: vm.currentSource,
  ...
  @ViewBuilder
  private func trashMenuItem(for asset: AssetRef, onTrashAssets: @escaping ([AssetRef.ID]) -> Void)
    -> some View
  {
  ...
  private func trashMenuTitle(for asset: AssetRef) -> String {
    #if os(macOS)
      let isLocalFilesystem =
        asset.primaryURL != nil && asset.catalog == nil && !(vm.currentSource is SMBSource)
...
private struct BrowseKeyboardShortcuts: ViewModifier {
  ...
  func body(content: Content) -> some View {
    content
      ...
      .onKeyPress(.return) {
        guard !vm.isSelecting, let asset = vm.selectedAsset, let assetRename else {
          return .ignored
        }
...
extension View {
  fileprivate func keyboardShortcuts(
    vm: BrowseViewModel,
...
#Preview("Photos access needed") {
  _BrowseGridPreviewWrapper(
    vm: BrowseViewModel.preview(.photosAuthNeeded),
    grantPhotos: true)
}
