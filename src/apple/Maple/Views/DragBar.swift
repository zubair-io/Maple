...
struct DragBar: View {
  ...
  private func makeDragGesture(barWidth: CGFloat) -> some Gesture {
    DragGesture(minimumDistance: 0)
      ...
      .onChanged { gesture in
        updateDrag(translation: gesture.translation.width, barWidth: barWidth)
      }
      .onEnded { gesture in
  ...
  private func updateDrag(translation: CGFloat, barWidth: CGFloat) {
    if dragStartValue == nil {
    ...
    let delta = DragBarMath.valueDelta(
      forDeltaX: translation, barWidth: barWidth, sensitivity: sensitivity
    )
    let newValue = DragBarMath.clamp((dragStartValue ?? 0) + delta)
    emitHaptics(from: lastValue, to: newValue)
    if newValue != lastValue { state.setArmedInternalValue(newValue) }
    lastValue = newValue
  }
