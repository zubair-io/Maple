...
import XCTest
  ...
  final class SkinToneDemoUITests: XCTestCase {
    ...
    func testCreatingASkinMaskAndDraggingHueMovesTheCloudTowardTheSkinLine() throws {
      ...
      // 2. Arm Mask through the shared adaptive dock and add the person.
      app.buttons["editor-dock-tool-mask"].click()
      ...
      try waitFor(
        createButton, predicateFormat: "isEnabled == 1", timeout: 15,
      ...
      XCTAssertTrue(
        hueSlider.waitForExistence(timeout: 5), "Hue slider control not found inside its row")
    ...
    private func waitForAccessibilityValue(
      _ element: XCUIElement, contains substring: String, timeout: TimeInterval,
      ...
      throw fail(
        "accessibility value never reported '\(substring)' — got: \((element.value as? String) ?? "nil")"
      )
    }
