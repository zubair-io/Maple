...
Four model conditions are rejected at the entry (`tile/guards.rs`) with an `Err`, which the FFI maps to code `10` and the Apple caller turns into a whole-image fallback:
...
| a local-adjustment layer sets `dehaze` | the per-mask control (#3407) runs the identical global kernel on that layer's scratch copy, so it inherits the identical whole-frame dependency                                              |
