...
extension EditSession {
  ...
  func _scheduleRender(phase: RenderPhase) {
    editSessionSignposter.emitEvent("render request")
  ...
  func _scheduleRefine(gen requested: UInt64? = nil) {
    let actor = renderActor
    Task {
      await actor.scheduleRefine(expectedGeneration: requested) { [weak self] genAtSchedule in
        guard let self else { return }
  ...
  private func refineBody(gen: UInt64) async {
    ...
    // A failed native-detail await may resume after a newer slider tick.
