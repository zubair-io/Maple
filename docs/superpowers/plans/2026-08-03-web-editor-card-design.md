...
> **Correction applied during execution (human ruling).** Two defects in the task text below were found while implementing it, and both are verified against source:
>
> 1. **Grade belongs to Effects, not Colour.** `TOOLS_IN_GROUP.effects` contains `colorGrade` (`tool-model.ts:111`), and `armTool` sets `armedGroup = groupOf(tool)` (`editor-state.service.ts:237`). A single Colour-only row containing Grade would flip the group to Effects on selection and hide itself. The chip row is therefore **group-parameterised**: Colour shows `Basic · HSL · B&W`, Effects shows `Basic · Grade`, Light and Detail show none. Wherever the steps below say the row is Colour-only or list four Colour chips, this correction governs.
> 2. **Basic cannot route through `armGroup`.** `armGroup(g)` preserves the armed tool when it already belongs to `g` (`editor-state.service.ts:241-245`), which is correct for the dock's group buttons but makes `groupChange.emit('color')` a no-op in exactly the case Basic serves — escaping HSL or B&W. Basic must arm the group's first slider tool directly.
