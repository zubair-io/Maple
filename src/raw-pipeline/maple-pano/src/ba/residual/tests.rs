...
fn jacobians_match_finite_differences() {
    ...
    for (k1, k2) in [(0.0, 0.0), (-0.08, 0.02), (0.05, 0.0)] {
        for with_override in [false, true] {
            ...
            let (_, jac) = eval_with_jacobian(&state, &frames, &block).expect("block valid");
            ...
            let num_k1 = numeric_column(&state, &frames, &block, 1e-7, |s, eps| s.k1 += eps);
            assert_close(jac.d_k1, num_k1, "d_k1");
            let num_k2 = numeric_column(&state, &frames, &block, 1e-7, |s, eps| s.k2 += eps);
