...
describe('seedFaceStageSplit — face stage split seeding', () => {
  it('seeds face-detect/face-embed at v1 for assets whose old face stage completed', async () => {
    if (!mongoReachable) return;
    const { seedFaceStageSplit } = await import('./migrations.ts');
    const { ObjectId } = await import('mongodb');

    const completedId = new ObjectId();
    await db!.collection('assets').insertOne({
      _id: completedId,
      stages: { face: { version: 1, attempts: 0, last_error: null, dead: false } },
    } as never);

    const res = await seedFaceStageSplit(db!);
    expect(res.updated).toBe(1);

    const after = await db!.collection('assets').findOne({ _id: completedId });
    expect(after?.stages?.['face-detect']?.version).toBe(1);
    expect(after?.stages?.['face-embed']?.version).toBe(1);
    expect(after?.stages?.['face-detect']?.dead).toBe(false);
    // The legacy `face` entry is left untouched (not orphaned).
    expect(after?.stages?.face?.version).toBe(1);
  });

  it('does not seed assets whose old face stage never completed', async () => {
    if (!mongoReachable) return;
    const { seedFaceStageSplit } = await import('./migrations.ts');
    const { ObjectId } = await import('mongodb');
    ...
    await db!.collection('assets').insertOne({
      _id: pendingId,
      stages: { face: { version: 0, attempts: 0, last_error: null, dead: false } },
    } as never);

    const res = await seedFaceStageSplit(db!);
    expect(res.updated).toBe(0);
    const after = await db!.collection('assets').findOne({ _id: pendingId });
    expect(after?.stages?.['face-detect']).toBeUndefined();
    expect(after?.stages?.['face-embed']).toBeUndefined();
  });

  it('is idempotent — a second run is a no-op (existing entries preserved)', async () => {
    if (!mongoReachable) return;
    const { seedFaceStageSplit } = await import('./migrations.ts');
    const { ObjectId } = await import('mongodb');
    ...
    expect((await seedFaceStageSplit(db!)).updated).toBe(1);
    // Simulate the embed stage having advanced past the seed before a
    // (spurious) second run; the $exists guard must leave it alone.
    await db!
      .collection('assets')
      .updateOne({ _id: id }, { $set: { 'stages.face-embed.version': 2 } });
    expect((await seedFaceStageSplit(db!)).updated).toBe(0);
    const after = await db!.collection('assets').findOne({ _id: id });
    expect(after?.stages?.['face-embed']?.version).toBe(2);
  });
});
