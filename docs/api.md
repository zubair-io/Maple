...
| `MAPLE_SQLITE_PATH`                                                                                                                                                   | **Absolute path to the library database.** This file _is_ the library — put it on a persistent volume and back it up the way the Mongo dump was. Defaults to `./data/maple.sqlite`, which is relative to the working directory and wrong for any service: in a container that resolves inside the image, the directory does not exist, and the server refuses to start |
| `MAPLE_MONGO_URI`, `MAPLE_MONGO_DB`                                                                                                                                   | Read **only** by the one-time boot migration, to import an existing MongoDB library. Nothing else touches MongoDB. Defaults `mongodb://localhost:27017`, `maple`                                                                                                                                                                                                       |
...
Rebuild it after any change to `raw-core` or `raw-ffi`. If the library is absent, `tryGetRawFfi()` returns null and every caller degrades: EVERY thumbnail/preview render — RAW and non-RAW bitmap alike — is logged as deferred and skipped (the pool's `requestId()` throws before a request ever reaches a child), and the rest of the pipeline advances. `ffi/raw_ffi.ts` is the symbol wrapper; `ffi/ffi-pool.ts` is the process pool described above. Since #3499 that same pool also renders non-RAW bitmap thumbnails (JPEG/PNG/WebP/TIFF/AVIF/HEIC/PSD/HDR) via the `@justmaple/maple` package instead of a second `sharp`/`imgdecode-pool.ts` process family, which is retired; `thumbs/hdr-decode-isolated.ts` still isolates Radiance HDR decode the same way as before. Pool size is an `app_settings` value (`_id: "performance"`) exposed at `GET/PATCH /api/workers/performance`.
...
docker run -p 3000:3000 \
  -v /opt/maple/data:/data -e MAPLE_SQLITE_PATH=/data/maple.sqlite \
...
**Before the first boot on an existing install, set `MAPLE_SQLITE_PATH` and mount its directory.** The server migrates from MongoDB on that boot and will not serve until it succeeds, so a path it cannot create is a server that does not start. The default is relative and resolves inside the container, where no such directory exists. A host that deploys by a hand-written `docker run` rather than compose needs both the `-v` and the `-e` added by hand; editing `docker-compose.yml` changes nothing there.
