 ...
 * the scene and activity facets — get generated columns with indexes, so those
 * two aggregations read an index instead of parsing a vision object per row.
 */
...
CREATE TABLE asset_detail (
  ...
  vision_scene_type TEXT GENERATED ALWAYS AS (json_extract(vision, '$.scene_type')) VIRTUAL,
  vision_activity   TEXT GENERATED ALWAYS AS (json_extract(vision, '$.activity')) VIRTUAL
) WITHOUT ROWID;
`;
