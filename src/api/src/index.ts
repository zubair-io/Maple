...
export function buildApp(_opts: { stageNames?: string[] } = {}): Elysia {
  const app = new Elysia()
    ...
    //
    // #2382: these MUST NOT be applied via `set.headers` to routes that return
    // a streamed file body. Elysia rebuilds a returned `Response` whenever
    // `set.headers` is non-empty, and that rebuild discards the `BunFile`
    // slice — so a 206 handed back the WHOLE file while still advertising a
    // partial `Content-Range`. AVFoundation reads that as a protocol violation
    // and aborts, which is why video played on no Apple device. Those routes
    // carry the same headers on the response itself (see `streamFileRange` /
    // `streamFile` in routes/library/shared.ts).
